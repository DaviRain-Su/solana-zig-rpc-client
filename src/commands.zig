...
const command_invoke = @import("./commands/invoke.zig");
...
fn appendAnchorPdaScalarSeed(
    ...
    const ParsePubkeyValue = struct {
        fn parse(value_inner: std.json.Value) ![]const u8 {
            switch (value_inner) {
                .string => return value_inner.string,
                .object => {
                    inline for (.{ "address", "publicKey", "pubkey", "key", "programId", "program_id" }) |field_name| {
                        if (value_inner.object.get(field_name)) |field_value| {
    ...
    const AppendEncodedBytes = struct {
        fn appendString(
            allocator_inner: Allocator,
            bytes_inner: *std.ArrayListUnmanaged(u8),
            encoded_value: []const u8,
        ) !bool {
            if (std.mem.startsWith(u8, encoded_value, "hex:")) {
                const hex_value = encoded_value[4..];
                if (hex_value.len % 2 != 0) return error.InvalidCli;
                const decoded = try allocator_inner.alloc(u8, hex_value.len / 2);
                defer allocator_inner.free(decoded);
                _ = std.fmt.hexToBytes(decoded, hex_value) catch return error.InvalidCli;
                try bytes_inner.appendSlice(allocator_inner, decoded);
                return true;
            }
            if (std.mem.startsWith(u8, encoded_value, "0x")) {
                const hex_value = encoded_value[2..];
                if (hex_value.len % 2 != 0) return error.InvalidCli;
                const decoded = try allocator_inner.alloc(u8, hex_value.len / 2);
                defer allocator_inner.free(decoded);
                _ = std.fmt.hexToBytes(decoded, hex_value) catch return error.InvalidCli;
            ...
            if (std.mem.startsWith(u8, encoded_value, "base64:")) {
                const base64_value = encoded_value[7..];
                const decoded_len = std.base64.standard.Decoder.calcSizeForSlice(base64_value) catch return error.InvalidCli;
                const decoded = try allocator_inner.alloc(u8, decoded_len);
                defer allocator_inner.free(decoded);
                try std.base64.standard.Decoder.decode(decoded, base64_value);
                try bytes_inner.appendSlice(allocator_inner, decoded);
                return true;
            }
            return false;
        ...
        fn appendObject(
            ...
            if (object_value.get("hex")) |field_value| {
                if (field_value != .string) return error.InvalidCli;
                const wrapped = try std.mem.concat(allocator_inner, u8, &.{ "hex:", field_value.string });
                defer allocator_inner.free(wrapped);
                _ = try appendString(allocator_inner, bytes_inner, wrapped);
                return true;
            }
            if (object_value.get("base64")) |field_value| {
                if (field_value != .string) return error.InvalidCli;
                const wrapped = try std.mem.concat(allocator_inner, u8, &.{ "base64:", field_value.string });
                defer allocator_inner.free(wrapped);
                _ = try appendString(allocator_inner, bytes_inner, wrapped);
                return true;
            }
            if (object_value.get("utf8")) |field_value| {
                if (field_value != .string) return error.InvalidCli;
                try bytes_inner.appendSlice(allocator_inner, field_value.string);
                return true;
            }
            return false;
        }
    ...
    if (std.mem.eql(u8, concrete_type.string, "bytes")) {
        switch (value) {
            .string => {
                if (try AppendEncodedBytes.appendString(allocator, bytes, value.string)) return;
                try bytes.appendSlice(allocator, value.string);
                return;
            },
            .object => {
                if (try AppendEncodedBytes.appendObject(allocator, bytes, value.object)) return;
                return error.InvalidCli;
            },
            .array => {
...
fn resolveAnchorIdlPdaAccountFieldType(
    ...
    if (std.mem.eql(u8, account_value.string, "tokenAccount") or
        std.mem.eql(u8, account_value.string, "TokenAccount") or
        std.mem.eql(u8, account_value.string, "token_account"))
    {
    ...
    if (std.mem.eql(u8, account_value.string, "mint") or
        std.mem.eql(u8, account_value.string, "Mint") or
        std.mem.eql(u8, account_value.string, "mint_account"))
    {
        const field_name, const child_path = if (std.mem.indexOfScalar(u8, path, '.')) |dot_index|
            .{ path[0..dot_index], path[dot_index + 1 ..] }
        else
    ...
    if (std.mem.eql(u8, account_value.string, "multisig") or
        std.mem.eql(u8, account_value.string, "Multisig") or
        std.mem.eql(u8, account_value.string, "multi_sig"))
    {
        const field_name, const child_path = if (std.mem.indexOfScalar(u8, path, '.')) |dot_index|
            .{ path[0..dot_index], path[dot_index + 1 ..] }
        else
...
fn parseAnchorIdlPathIndex(path_segment: []const u8) !usize {
    if (path_segment.len == 0) return error.InvalidCli;
    return std.fmt.parseInt(usize, path_segment, 10) catch return error.InvalidCli;
}

fn resolveAnchorIdlFieldTypeSpec(field_value: std.json.Value) !std.json.Value {
    if (field_value == .object) {
        if (field_value.object.get("type")) |field_type| return field_type;
    }
    return field_value;
...
fn resolveAnchorIdlArrayElementType(type_spec: std.json.Value) !std.json.Value {
    if (type_spec != .object) return error.InvalidCli;
    const array_value = type_spec.object.get("array") orelse return error.InvalidCli;
    return switch (array_value) {
        .array => |items| blk: {
            if (items.items.len != 2) return error.InvalidCli;
            break :blk items.items[0];
        },
        .object => |object_value| object_value.get("type") orelse return error.InvalidCli,
        else => return error.InvalidCli,
    };
}
...
fn resolveAnchorIdlPdaSeedType(
    ...
    if (concrete_type.object.get("option")) |child_type| {
        return try resolveAnchorIdlPdaSeedType(idl, child_type, path);
    }
    if (concrete_type.object.get("vec")) |child_type| {
        _ = try parseAnchorIdlPathIndex(field_name);
        return try resolveAnchorIdlPdaSeedType(idl, child_type, child_path);
    }
    if (concrete_type.object.get("array") != null) {
        const child_type = try resolveAnchorIdlArrayElementType(concrete_type);
        _ = try parseAnchorIdlPathIndex(field_name);
        return try resolveAnchorIdlPdaSeedType(idl, child_type, child_path);
    }
    ...
    if (std.mem.eql(u8, kind_value.string, "struct")) {
        ...
        if (fields_value.array.items.len == 0) return error.InvalidCli;
        const first_field = fields_value.array.items[0];
        if (!(first_field == .object and first_field.object.get("name") != null)) {
            const field_index = try parseAnchorIdlPathIndex(field_name);
            if (field_index >= fields_value.array.items.len) return error.InvalidCli;
            return try resolveAnchorIdlPdaSeedType(
                idl,
                try resolveAnchorIdlFieldTypeSpec(fields_value.array.items[field_index]),
                child_path,
            );
        }
    ...
    if (std.mem.eql(u8, kind_value.string, "enum")) {
        ...
        for (variants_value.array.items) |variant_value| {
            ...
            const variant_name = variant_value.object.get("name") orelse return error.InvalidCli;
            if (variant_name != .string) return error.InvalidCli;
            if (!anchorIdlEnumVariantNameMatches(variant_name.string, field_name)) continue;

            const fields_value = variant_value.object.get("fields") orelse return error.InvalidCli;
            if (fields_value == .null or fields_value != .array) return error.InvalidCli;
            if (child_path.len == 0) return error.InvalidCli;
            if (fields_value.array.items.len == 0) return error.InvalidCli;

            const first_field = fields_value.array.items[0];
            if (!(first_field == .object and first_field.object.get("name") != null)) {
                const field_index = try parseAnchorIdlPathIndex(nested_field_name);
                if (field_index >= fields_value.array.items.len) return error.InvalidCli;
                return try resolveAnchorIdlPdaSeedType(
                    idl,
                    try resolveAnchorIdlFieldTypeSpec(fields_value.array.items[field_index]),
...
fn encodeAnchorPdaArgSeed(
    ...
    for (instruction.args) |arg| {
        if (anchorIdlPathSegmentMatches(arg.name, arg_name)) {
            arg_type = arg.type;
            break;
        }
    }
...
fn findJsonBindingValue(
    ...
    const FindObjectValue = struct {
        fn matches(candidate: []const u8, key: []const u8) bool {
            if (std.mem.eql(u8, candidate, key)) return true;
            if (key.len == 0) return false;

            if (candidate.len == key.len and candidate.len != 0) {
                if (std.ascii.toLower(candidate[0]) == std.ascii.toLower(key[0]) and
                    std.mem.eql(u8, candidate[1..], key[1..]))
                {
                    return true;
                }
            }
            ...
            while (true) {
                while (candidate_index < candidate.len and candidate[candidate_index] == '_') {
                    candidate_index += 1;
                }
                while (key_index < key.len and key[key_index] == '_') {
                    key_index += 1;
                }

                if (candidate_index == candidate.len or key_index == key.len) break;
                if (std.ascii.toLower(candidate[candidate_index]) != std.ascii.toLower(key[key_index])) return false;
            ...
            return candidate_index == candidate.len and key_index == key.len;
        }
    ...
    switch (json_account_bindings.*) {
        ...
        .array => {
            if (std.mem.indexOfScalar(u8, path, '.')) |dot_index| {
                ...
                if (index >= json_account_bindings.array.items.len) return null;
                const nested_value = json_account_bindings.array.items[index];
                return findJsonBindingValue(&nested_value, tail);
            }

            const index = std.fmt.parseInt(usize, path, 10) catch return null;
            if (index >= json_account_bindings.array.items.len) return null;
            return json_account_bindings.array.items[index];
        },
        else => return null,
    }
}
...
fn loadAnchorIdlInvokeInstructionSpecWithOptions(
    ...
    return loadAnchorIdlInvokeInstructionSpecWithOptionsWithPayerSecret(
        allocator,
        idl_arg,
        instruction_name,
...
fn buildCliSimulationOptions(
    ...
    const simulation_min_context_slot = if (simulation_min_context_slot_arg) |value|
        ...
        null;
    const simulation_accounts_options = if (simulation_accounts.len > 0)
        client.SimulationAccountsOptions{
            .addresses = simulation_accounts,
            .encoding = simulation_account_encoding,
        }
    else
        null;

    return if (simulate_sig_verify or
...
const CliInvokeExecutionArgs = command_invoke.CliInvokeExecutionArgs;
const CliInvokeCommandBehavior = command_invoke.CliInvokeCommandBehavior;
const CliInvokeCommandSpec = command_invoke.CliInvokeCommandSpec;
const cli_invoke_command_specs = command_invoke.cli_invoke_command_specs;
const buildCliInvokeContextArgs = command_invoke.buildCliInvokeContextArgs;
const buildCliInvokePayloadArgs = command_invoke.buildCliInvokePayloadArgs;
const buildCliInvokeExecutionArgs = command_invoke.buildCliInvokeExecutionArgs;
const lookupInvokeCommandSpec = command_invoke.lookupInvokeCommandSpec;
...
fn loadProgramInvokeInstructionSpec(
    ...
    return loadProgramInvokeInstructionSpecWithPayerSecret(
        allocator,
        program_id,
        accounts_arg,
        data_arg,
...
fn loadProgramInvokeInstructionSpecWithPayerSecret(
    allocator: Allocator,
    program_id: []const u8,
    accounts_arg: []const u8,
    data_arg: ?[]const u8,
...
pub fn runCommand(allocator: Allocator, rpc: *client.RpcClient, args: *const cli.ParsedArgs) !void {
    ...
    const output_json = args.output_json;
    ...
    if (output_json and
        command != .preview_instructions and
        command != .preview_program_invoke and
        command != .preview_idl_invoke)
    {
        reportInvalidCliMessage("error: --json requires preview-instructions, preview-program-invoke, or preview-idl-invoke\n", .{});
        return error.InvalidCli;
    }
    ...
    {
        reportInvalidCliMessage(
            "error: simulation query options require simulate-transaction, simulate-instructions, simulate-versioned-instructions, simulate-program-invoke, simulate-versioned-program-invoke, simulate-idl-invoke, or simulate-versioned-idl-invoke\n",
            .{},
        );
        return error.InvalidCli;
    }
...
test "runCommand simulate-instructions supports sender-secret-key flag" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    try sender_context.sender.pushResultJson(
    ...
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-instructions-secret-key" },
    );
    defer rpc.deinit();
    ...
    const spec_json = try std.fmt.allocPrint(
        allocator,
        \\{{"recent_blockhash":"{s}","instructions":[{{"program_id":"11111111111111111111111111111111","accounts":[{{"pubkey":"{s}","is_signer":true,"is_writable":true}},{{"pubkey":"{s}","is_signer":false,"is_writable":true}}],"data":"ping","data_encoding":"utf8"}}]}}
    ,
...
test "runCommand send-program-invoke accepts sender-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    for (&latest_blockhash_bytes, 0..) |*byte, index| byte.* = @intCast(index + 44);
    const latest_blockhash = try client.encodeBase58(allocator, &latest_blockhash_bytes);
    defer allocator.free(latest_blockhash);
    try sender_context.sender.pushLatestBlockhashResponse(
        46,
    ...
    try sender_context.sender.pushResultJson("\"SigSecret1212121212121212121212121212121212121212121212121212121212121212\"");
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://send-program-invoke-secret" },
    );
    defer rpc.deinit();
...
test "runCommand simulate-program-invoke accepts sender-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    for (&latest_blockhash_bytes, 0..) |*byte, index| byte.* = @intCast(index + 77);
    const latest_blockhash = try client.encodeBase58(allocator, &latest_blockhash_bytes);
    defer allocator.free(latest_blockhash);
    try sender_context.sender.pushLatestBlockhashResponse(
        46,
    ...
    try sender_context.sender.pushResultJson(
        \\{"context":{"slot":18},"value":{"accounts":[],"err":null,"fee":120,"unitsConsumed":22,"logs":["Program log: secret-invoke-ok"]}}
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-program-invoke-secret" },
    );
    defer rpc.deinit();
    ...
    try expectMockSenderLastCapturedRequestMethod(&sender_context.sender, "simulateTransaction");
    try expectMockSenderScriptSatisfied(&sender_context.sender);
    try std.testing.expect(std.mem.indexOf(u8, captured, "Program log: secret-invoke-ok") != null);
}

test "runCommand simulate-versioned-program-invoke accepts sender-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    for (&latest_blockhash_bytes, 0..) |*byte, index| byte.* = @intCast(index + 167);
    const latest_blockhash = try client.encodeBase58(allocator, &latest_blockhash_bytes);
    defer allocator.free(latest_blockhash);
    try sender_context.sender.pushLatestBlockhashResponse(
        55,
    ...
    try sender_context.sender.pushResultJson(
        \\{"context":{"slot":19},"value":{"accounts":[],"err":null,"fee":140,"unitsConsumed":24,"logs":["Program log: versioned-secret-invoke-ok"]}}
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-versioned-program-invoke-secret" },
    );
    defer rpc.deinit();
    ...
    try expectMockSenderLastCapturedRequestMethod(&sender_context.sender, "simulateTransaction");
    try expectMockSenderScriptSatisfied(&sender_context.sender);
    try std.testing.expect(std.mem.indexOf(u8, captured, "Program log: versioned-secret-invoke-ok") != null);
}

test "runCommand simulate-versioned-idl-invoke supports sig verify and inner instruction options" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    for (&latest_blockhash_bytes, 0..) |*byte, index| byte.* = @intCast(index + 137);
    const latest_blockhash = try client.encodeBase58(allocator, &latest_blockhash_bytes);
    defer allocator.free(latest_blockhash);
    try sender_context.sender.pushLatestBlockhashResponse(
        59,
    ...
    try sender_context.sender.pushResultJson(
        \\{"context":{"slot":20},"value":{"accounts":[],"err":null,"fee":142,"unitsConsumed":19,"logs":["Program log: versioned-idl-options-ok"]}}
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-versioned-idl-invoke-options" },
    );
    defer rpc.deinit();
    ...
    try std.testing.expect(std.mem.indexOf(u8, commandCapturedRequest(&sender_context), "\"sigVerify\":true") != null);
    try std.testing.expect(std.mem.indexOf(u8, commandCapturedRequest(&sender_context), "\"innerInstructions\":true") != null);
    try std.testing.expect(std.mem.indexOf(u8, captured, "Program log: versioned-idl-options-ok") != null);
}
...
test "runCommand simulate-idl-invoke accepts sender-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    ...
    try sender_context.sender.pushResultJson(
        \\{"context":{"slot":49},"value":{"accounts":[],"err":null,"fee":108,"unitsConsumed":18,"logs":["Program log: idl-secret-ok"]}}
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-idl-invoke-secret-key" },
    );
    defer rpc.deinit();
    ...
    try expectMockSenderLastCapturedRequestMethod(&sender_context.sender, "simulateTransaction");
    try expectMockSenderScriptSatisfied(&sender_context.sender);
    try std.testing.expect(std.mem.indexOf(u8, captured, "Program log: idl-secret-ok") != null);
}

test "runCommand simulate-idl-invoke accepts additional-signer-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    ...
    try sender_context.sender.pushResultJson(
        \\{"context":{"slot":49},"value":{"accounts":[],"err":null,"fee":108,"unitsConsumed":18,"logs":["Program log: idl-extra-signer-ok"]}}
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-idl-invoke-extra-signer-secret" },
    );
    defer rpc.deinit();
    ...
    const payer_raw = try Ed25519.KeyPair.generateDeterministic(.{65} ** 32);
    const payer_secret_key = payer_raw.secret_key.toBytes();
    const payer_secret_key_base58 = try client.encodeBase58(allocator, &payer_secret_key);
    defer allocator.free(payer_secret_key_base58);

    const extra_signer_raw = try Ed25519.KeyPair.generateDeterministic(.{66} ** 32);
    const extra_signer_secret_key = extra_signer_raw.secret_key.toBytes();
    const extra_signer_secret_key_base58 = try client.encodeBase58(allocator, &extra_signer_secret_key);
    defer allocator.free(extra_signer_secret_key_base58);
    const extra_signer_pubkey_bytes = extra_signer_raw.public_key.toBytes();
...
test "runCommand simulate-versioned-idl-invoke accepts sender-secret-key" {
    ...
    defer sender_context.deinit();
    var latest_blockhash_bytes: [32]u8 = undefined;
    for (&latest_blockhash_bytes, 0..) |*byte, index| byte.* = @intCast(index + 151);
    const latest_blockhash = try client.encodeBase58(allocator, &latest_blockhash_bytes);
    defer allocator.free(latest_blockhash);
    try sender_context.sender.pushLatestBlockhashResponse(
        28,
    ...
    try sender_context.sender.pushResultJson(
        \\{"context":{"slot":58},"value":{"accounts":[],"err":null,"fee":208,"unitsConsumed":21,"logs":["Program log: versioned-idl-secret-ok"]}}
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-versioned-idl-invoke-secret-key" },
    );
    defer rpc.deinit();
...
test "loadAnchorIdlInvokeInstructionSpec accepts TokenAccount seed account alias" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-token-account-alias-seed-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const token = client.Pubkey.fromBytes(.{193} ** 32);
    const token_base58 = try token.toBase58(allocator);
    defer allocator.free(token_base58);
    const owner = client.Pubkey.fromBytes(.{194} ** 32);
    const owner_base58 = try owner.toBase58(allocator);
    defer allocator.free(owner_base58);
    const program_id = client.Pubkey.fromBytes(.{195} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);

    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[58,58,58,58,58,58,58,58],"accounts":[{{"name":"token"}},{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"account","path":"token.owner","account":"TokenAccount"}}]}}}}],"args":[]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec accepts Mint seed account alias" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-mint-alias-seed-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const mint = client.Pubkey.fromBytes(.{197} ** 32);
    const mint_base58 = try mint.toBase58(allocator);
    defer allocator.free(mint_base58);
    const program_id = client.Pubkey.fromBytes(.{198} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);

    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[59,59,59,59,59,59,59,59],"accounts":[{{"name":"mint"}},{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"account","path":"mint.is_initialized","account":"Mint"}}]}}}}],"args":[]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec accepts Multisig seed account alias" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-multisig-alias-seed-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const multisig = client.Pubkey.fromBytes(.{200} ** 32);
    const multisig_base58 = try multisig.toBase58(allocator);
    defer allocator.free(multisig_base58);
    const program_id = client.Pubkey.fromBytes(.{201} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);

    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[60,60,60,60,60,60,60,60],"accounts":[{{"name":"multisig"}},{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"account","path":"multisig.m","account":"Multisig"}}]}}}}],"args":[]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from base64 bytes arg seed" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-bytes-base64-arg-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const program_id = client.Pubkey.fromBytes(.{190} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);

    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[75,75,75,75,75,75,75,75],"accounts":[{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"arg","path":"digest"}}]}}}}],"args":[{{"name":"digest","type":"bytes"}}]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from hex bytes account seed field via cli literal" {
    const allocator = std.testing.allocator;

    const payer_raw = try Ed25519.KeyPair.generateDeterministic(.{191} ** 32);
    const payer_secret_key = payer_raw.secret_key.toBytes();
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-bytes-hex-account-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{192} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    ...
    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[76,76,76,76,76,76,76,76],"accounts":[{{"name":"state"}},{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"account","path":"state.digest","type":"bytes"}}]}}}}],"args":[]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
    ...
    var loaded = try loadAnchorIdlInvokeInstructionSpec(
        ...
        null,
        null,
        &.{ state_binding, "state.digest=hex:01020304" },
        &.{},
        null,
        payer_keypair_realpath,
    );
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from hex bytes arg object" {
    const allocator = std.testing.allocator;

    const payer_raw = try Ed25519.KeyPair.generateDeterministic(.{194} ** 32);
    const payer_secret_key = payer_raw.secret_key.toBytes();
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-bytes-hex-object-arg-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[77,77,77,77,77,77,77,77],"accounts":[{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"arg","path":"digest"}}]}}}}],"args":[{{"name":"digest","type":"bytes"}}]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from base64 bytes account seed field via cli object literal" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-bytes-base64-object-account-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{197} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const program_id = client.Pubkey.fromBytes(.{198} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    ...
    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[78,78,78,78,78,78,78,78],"accounts":[{{"name":"state"}},{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":[118,97,117,108,116]}},{{"kind":"account","path":"state.digest","type":"bytes"}}]}}}}],"args":[]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
    ...
    var loaded = try loadAnchorIdlInvokeInstructionSpec(
        ...
        null,
        null,
        &.{ state_binding, "state.digest={\"base64\":\"AQIDBA==\"}" },
        &.{},
        null,
        payer_keypair_realpath,
    );
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from publicKey arg object" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-pubkey-arg-object-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const authority = client.Pubkey.fromBytes(.{205} ** 32);
    const authority_base58 = try authority.toBase58(allocator);
    defer allocator.free(authority_base58);
    const program_id = client.Pubkey.fromBytes(.{206} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    const args_json = try std.fmt.allocPrint(
        allocator,
    ...
    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[81,81,81,81,81,81,81,81],"accounts":[{{"name":"vault","writable":true,"pda":{{"seeds":[{{"kind":"const","value":"vault"}},{{"kind":"arg","path":"authority"}}]}}}}],"args":[{{"name":"authority","type":"publicKey"}}]}}]}}
    ,
        .{program_id_base58},
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA with typed const program object" {
    const allocator = std.testing.allocator;

    const payer_raw = try Ed25519.KeyPair.generateDeterministic(.{207} ** 32);
    const payer_secret_key = payer_raw.secret_key.toBytes();
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-const-program-object-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const base_program_id = client.Pubkey.fromBytes(.{208} ** 32);
    const base_program_id_base58 = try base_program_id.toBase58(allocator);
    defer allocator.free(base_program_id_base58);
    const pda_program_id = client.Pubkey.fromBytes(.{209} ** 32);
    const pda_program_id_base58 = try pda_program_id.toBase58(allocator);
    defer allocator.free(pda_program_id_base58);

    const idl_json = try std.fmt.allocPrint(
        allocator,
        \\{{"address":"{s}","instructions":[{{"name":"init","discriminator":[82,82,82,82,82,82,82,82],"accounts":[{{"name":"state","writable":true,"pda":{{"program":{{"kind":"const","type":"publicKey","value":{{"address":"{s}"}}}},"seeds":[{{"kind":"const","value":"state"}}]}}}}],"args":[]}}]}}
    ,
        .{ base_program_id_base58, pda_program_id_base58 },
    );
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from optional nested arg path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-optional-nested-arg-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[83,83,83,83,83,83,83,83],\"accounts\":[{\"name\":\"state\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"state\"},{\"kind\":\"arg\",\"path\":\"config.counter\"}]}}],\"args\":[{\"name\":\"config\",\"type\":{\"option\":{\"defined\":{\"name\":\"Config\"}}}}]}],\"types\":[{\"name\":\"Config\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"counter\",\"type\":\"u64\"}]}}]}",
    });
    defer allocator.free(idl_json);
    ...
    var counter_seed: [8]u8 = undefined;
    std.mem.writeInt(u64, &counter_seed, 42, .little);
    const expected_pda = try findProgramAddress(
        allocator,
        &.{ "state", &counter_seed },
        program_id,
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from optional nested account path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-optional-nested-account-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{213} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const program_id = client.Pubkey.fromBytes(.{214} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[84,84,84,84,84,84,84,84],\"accounts\":[{\"name\":\"state\"},{\"name\":\"vault\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"vault\"},{\"kind\":\"account\",\"path\":\"state.config.counter\",\"account\":\"State\"}]}}],\"args\":[]}],\"accounts\":[{\"name\":\"State\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"config\",\"type\":{\"option\":{\"defined\":{\"name\":\"Config\"}}}}]}}],\"types\":[{\"name\":\"Config\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"counter\",\"type\":\"u64\"}]}}]}",
    });
    defer allocator.free(idl_json);
    const accounts_json = try std.fmt.allocPrint(
        allocator,
        "{{\"state\":{{\"address\":\"{s}\",\"config\":{{\"counter\":\"99\"}}}}}}",
        .{state_base58},
    );
    defer allocator.free(accounts_json);
    ...
    var counter_seed: [8]u8 = undefined;
    std.mem.writeInt(u64, &counter_seed, 99, .little);
    const expected_pda = try findProgramAddress(
        allocator,
        &.{ "vault", &counter_seed },
        program_id,
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from nested enum arg path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-nested-enum-arg-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const program_id = client.Pubkey.fromBytes(.{216} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[85,85,85,85,85,85,85,85],\"accounts\":[{\"name\":\"state\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"state\"},{\"kind\":\"arg\",\"path\":\"threshold.fixed.value\"}]}}],\"args\":[{\"name\":\"threshold\",\"type\":{\"defined\":{\"name\":\"Threshold\"}}}]}],\"types\":[{\"name\":\"Threshold\",\"type\":{\"kind\":\"enum\",\"variants\":[{\"name\":\"Fixed\",\"fields\":[{\"name\":\"value\",\"type\":\"u16\"}]},{\"name\":\"Open\"}]}}]}",
    });
    defer allocator.free(idl_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from nested enum account path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-nested-enum-account-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{218} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const program_id = client.Pubkey.fromBytes(.{219} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[86,86,86,86,86,86,86,86],\"accounts\":[{\"name\":\"state\"},{\"name\":\"vault\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"vault\"},{\"kind\":\"account\",\"path\":\"state.threshold.fixed.value\",\"account\":\"State\"}]}}],\"args\":[]}],\"accounts\":[{\"name\":\"State\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"threshold\",\"type\":{\"defined\":{\"name\":\"Threshold\"}}}]}}],\"types\":[{\"name\":\"Threshold\",\"type\":{\"kind\":\"enum\",\"variants\":[{\"name\":\"Fixed\",\"fields\":[{\"name\":\"value\",\"type\":\"u16\"}]},{\"name\":\"Open\"}]}}]}",
    });
    defer allocator.free(idl_json);
    const accounts_json = try std.fmt.allocPrint(
        allocator,
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from tuple struct arg path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-tuple-struct-arg-path-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const program_id = client.Pubkey.fromBytes(.{221} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[87,87,87,87,87,87,87,87],\"accounts\":[{\"name\":\"state\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"state\"},{\"kind\":\"arg\",\"path\":\"pair.1\"}]}}],\"args\":[{\"name\":\"pair\",\"type\":{\"defined\":{\"name\":\"Pair\"}}}]}],\"types\":[{\"name\":\"Pair\",\"type\":{\"kind\":\"struct\",\"fields\":[\"u8\",\"u16\"]}}]}",
    });
    defer allocator.free(idl_json);
    ...
    var field_seed: [2]u8 = undefined;
    std.mem.writeInt(u16, &field_seed, 1025, .little);
    const expected_pda = try findProgramAddress(
        allocator,
        &.{ "state", &field_seed },
        program_id,
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from tuple enum account path" {
    const allocator = std.testing.allocator;

    const payer_raw = try Ed25519.KeyPair.generateDeterministic(.{222} ** 32);
    const payer_secret_key = payer_raw.secret_key.toBytes();
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-tuple-enum-account-path-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{223} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const program_id = client.Pubkey.fromBytes(.{224} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[88,88,88,88,88,88,88,88],\"accounts\":[{\"name\":\"state\"},{\"name\":\"vault\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"vault\"},{\"kind\":\"account\",\"path\":\"state.mode.values.1\",\"account\":\"State\"}]}}],\"args\":[]}],\"accounts\":[{\"name\":\"State\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"mode\",\"type\":{\"defined\":{\"name\":\"Mode\"}}}]}}],\"types\":[{\"name\":\"Mode\",\"type\":{\"kind\":\"enum\",\"variants\":[{\"name\":\"Values\",\"fields\":[\"u8\",\"u16\"]}]}}]}",
    });
    defer allocator.free(idl_json);
    const accounts_json = try std.fmt.allocPrint(
        allocator,
        "{{\"state\":{{\"address\":\"{s}\",\"mode\":{{\"values\":[7,1025]}}}}}}",
        .{state_base58},
    );
    defer allocator.free(accounts_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from vec nested arg path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-vec-nested-arg-path-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[89,89,89,89,89,89,89,89],\"accounts\":[{\"name\":\"state\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"state\"},{\"kind\":\"arg\",\"path\":\"configs.1.counter\"}]}}],\"args\":[{\"name\":\"configs\",\"type\":{\"vec\":{\"defined\":{\"name\":\"Config\"}}}}]}],\"types\":[{\"name\":\"Config\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"counter\",\"type\":\"u16\"}]}}]}",
    });
    defer allocator.free(idl_json);
    ...
    var loaded = try loadAnchorIdlInvokeInstructionSpec(
        ...
        idl_json,
        "init",
        "{\"configs\":[{\"counter\":7},{\"counter\":1025}]}",
        null,
        &.{},
        &.{},
        null,
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from array nested account path" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-array-nested-account-path-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{228} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const program_id = client.Pubkey.fromBytes(.{229} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[90,90,90,90,90,90,90,90],\"accounts\":[{\"name\":\"state\"},{\"name\":\"vault\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"vault\"},{\"kind\":\"account\",\"path\":\"state.configs.1.counter\",\"account\":\"State\"}]}}],\"args\":[]}],\"accounts\":[{\"name\":\"State\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"configs\",\"type\":{\"array\":[{\"defined\":{\"name\":\"Config\"}},2]}}]}}],\"types\":[{\"name\":\"Config\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"counter\",\"type\":\"u16\"}]}}]}",
    });
    defer allocator.free(idl_json);
    const accounts_json = try std.fmt.allocPrint(
        allocator,
        "{{\"state\":{{\"address\":\"{s}\",\"configs\":[{{\"counter\":7}},{{\"counter\":1025}}]}}}}",
        .{state_base58},
    );
    defer allocator.free(accounts_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from snake case arg json key" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-snake-arg-json-key-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const authority = client.Pubkey.fromBytes(.{236} ** 32);
    const authority_base58 = try authority.toBase58(allocator);
    defer allocator.free(authority_base58);
    const program_id = client.Pubkey.fromBytes(.{237} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[93,93,93,93,93,93,93,93],\"accounts\":[{\"name\":\"state\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"state\"},{\"kind\":\"arg\",\"path\":\"config.mintAuthority\"}]}}],\"args\":[{\"name\":\"config\",\"type\":{\"defined\":{\"name\":\"Config\"}}}]}],\"types\":[{\"name\":\"Config\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"mintAuthority\",\"type\":\"publicKey\"}]}}]}",
    });
    defer allocator.free(idl_json);
    const args_json = try std.fmt.allocPrint(
        allocator,
        "{{\"config\":{{\"mint_authority\":\"{s}\"}}}}",
        .{authority_base58},
    );
    defer allocator.free(args_json);
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from snake case arg path alias" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-snake-arg-path-alias-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[99,99,99,99,99,99,99,99],\"accounts\":[{\"name\":\"state\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"state\"},{\"kind\":\"arg\",\"path\":\"mint_authority\"}]}}],\"args\":[{\"name\":\"mintAuthority\",\"type\":\"publicKey\"}]}]}",
    });
    defer allocator.free(idl_json);
    const args_json = try std.fmt.allocPrint(
        allocator,
...
test "loadAnchorIdlInvokeInstructionSpec derives PDA from camel case account json key" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-pda-camel-account-json-key-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{239} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const authority = client.Pubkey.fromBytes(.{240} ** 32);
    const authority_base58 = try authority.toBase58(allocator);
    defer allocator.free(authority_base58);
    const program_id = client.Pubkey.fromBytes(.{241} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[94,94,94,94,94,94,94,94],\"accounts\":[{\"name\":\"state\"},{\"name\":\"vault\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"vault\"},{\"kind\":\"account\",\"path\":\"state.mint_authority\",\"account\":\"State\"}]}}],\"args\":[]}],\"accounts\":[{\"name\":\"State\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"mint_authority\",\"type\":\"publicKey\"}]}}]}",
    });
    defer allocator.free(idl_json);
    const accounts_json = try std.fmt.allocPrint(
        allocator,
        "{{\"state\":{{\"address\":\"{s}\",\"mintAuthority\":\"{s}\"}}}}",
...
test "loadAnchorIdlInvokeInstructionSpec derives pda from cli raw account path alias" {
    const allocator = std.testing.allocator;
    ...
    const payer_keypair_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/test-idl-cli-raw-account-path-alias-payer-{d}.json",
        .{std.time.nanoTimestamp()},
    );
    defer allocator.free(payer_keypair_path);
    defer std.fs.cwd().deleteFile(payer_keypair_path) catch {};
    ...
    const state = client.Pubkey.fromBytes(.{252} ** 32);
    const state_base58 = try state.toBase58(allocator);
    defer allocator.free(state_base58);
    const authority = client.Pubkey.fromBytes(.{253} ** 32);
    const authority_base58 = try authority.toBase58(allocator);
    defer allocator.free(authority_base58);
    const state_binding = try std.fmt.allocPrint(allocator, "state={s}", .{state_base58});
    defer allocator.free(state_binding);
    const authority_binding = try std.fmt.allocPrint(allocator, "state.mint_authority={s}", .{authority_base58});
    defer allocator.free(authority_binding);
    const program_id = client.Pubkey.fromBytes(.{254} ** 32);
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    ...
    const idl_json = try std.mem.concat(allocator, u8, &.{
        "{\"address\":\"",
        program_id_base58,
        "\",\"instructions\":[{\"name\":\"init\",\"discriminator\":[98,98,98,98,98,98,98,98],\"accounts\":[{\"name\":\"state\"},{\"name\":\"vault\",\"writable\":true,\"pda\":{\"seeds\":[{\"kind\":\"const\",\"value\":\"vault\"},{\"kind\":\"account\",\"path\":\"state.mintAuthority\",\"account\":\"State\"}]}}],\"args\":[]}],\"accounts\":[{\"name\":\"State\",\"type\":{\"kind\":\"struct\",\"fields\":[{\"name\":\"mintAuthority\",\"type\":\"publicKey\"}]}}]}",
    });
    defer allocator.free(idl_json);
...
test "runCommand send-idl-invoke accepts sender-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    try sender_context.sender.pushResultJson(
        "\"SigIdlSecret1111111111111111111111111111111111111111111111111111111111111\"",
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://send-idl-invoke-secret" },
    );
    defer rpc.deinit();
    ...
    try expectMockSenderScriptSatisfied(&sender_context.sender);
    try std.testing.expectEqualStrings(
        "signature: SigIdlSecret1111111111111111111111111111111111111111111111111111111111111\n",
        captured,
    );
}

test "runCommand send-idl-invoke accepts additional-signer-secret-key" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    try sender_context.sender.pushResultJson(
        "\"SigIdlExtraSig11111111111111111111111111111111111111111111111111111111111111\"",
    );
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://send-idl-invoke-extra-signer-secret" },
    );
    defer rpc.deinit();
    ...
    const extra_signer_raw = try Ed25519.KeyPair.generateDeterministic(.{62} ** 32);
    const extra_signer_secret_key = extra_signer_raw.secret_key.toBytes();
    const extra_signer_secret_key_base58 = try client.encodeBase58(allocator, &extra_signer_secret_key);
    defer allocator.free(extra_signer_secret_key_base58);
    const extra_signer_pubkey_bytes = extra_signer_raw.public_key.toBytes();
...
test "runCommand simulate-versioned-instructions supports sender-secret-key flag" {
    const allocator = std.testing.allocator;
    var sender_context = CommandTestSender.init(allocator);
    defer sender_context.deinit();
    try sender_context.sender.pushResultJson(
    ...
    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.fromMockSender(&sender_context.sender),
        .{ .endpoint = "command-test://simulate-versioned-instructions-secret-key" },
    );
    defer rpc.deinit();
    ...
    const spec_json = try std.fmt.allocPrint(
        allocator,
        \\{{"recent_blockhash":"{s}","address_lookup_tables":[{{"account_key":"{s}","addresses":["{s}"]}}],"instructions":[{{"program_id":"11111111111111111111111111111111","accounts":[{{"pubkey":"{s}","is_signer":true,"is_writable":true}},{{"pubkey":"{s}","is_signer":false,"is_writable":true}}],"data":"ping","data_encoding":"utf8"}}]}}
    ,
        .{ recent_blockhash, lookup_account_pubkey, destination_pubkey_base58, payer_pubkey_base58, destination_pubkey_base58 },
    );
    defer allocator.free(spec_json);
