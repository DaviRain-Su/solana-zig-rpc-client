...
pub const PreferredInvocationModeResolution = struct {
    requested_mode: ?InvocationMode,
    selected_mode: ?InvocationMode,
    requested_mode_buildable: bool,
    used_fallback: bool,
...
fn allocPreferredExecutionBytesValue(
    allocator: Allocator,
    bytes: []const u8,
    already_base64: bool,
) ![]u8 {
    if (already_base64) return try allocator.dupe(u8, bytes);
    const encoded = try allocator.alloc(u8, std.base64.standard.Encoder.calcSize(bytes.len));
    _ = std.base64.standard.Encoder.encode(encoded, bytes);
    return encoded;
}

fn writePreferredBytesExecutionResultJsonWithField(
    writer: *std.Io.Writer,
    allocator: Allocator,
    ...
    const output_value = try allocPreferredExecutionBytesValue(allocator, result.bytes, already_base64);
    defer allocator.free(output_value);
    ...
    try writeJsonBoolField(writer, &first, "used_fallback", result.execution_report.used_fallback);
    try writeJsonBoolField(writer, &first, "can_execute_selected_mode", result.execution_report.can_execute_selected_mode);
    try writeJsonStringField(writer, &first, field_name, output_value);
    try writeJsonUsizeField(writer, &first, "diagnostic_error_count", diagnostics.errorCount());
    try writeJsonUsizeField(writer, &first, "diagnostic_warning_count", diagnostics.warningCount());
    try writeJsonUsizeField(writer, &first, "diagnostic_info_count", diagnostics.infoCount());
    try writeJsonDiagnosticsField(writer, &first, diagnostics);
...
pub fn writePreferredMessageBytesExecutionResultJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
) !void {
...
pub fn allocPreferredMessageBytesExecutionResultJson(
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
) ![]u8 {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    errdefer aw.deinit();
    try writePreferredMessageBytesExecutionResultJson(&aw.writer, allocator, result);
    return try aw.toOwnedSlice();
}

pub fn writePreferredMessageBase64ExecutionResultJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
) !void {
...
pub fn allocPreferredMessageBase64ExecutionResultJson(
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
) ![]u8 {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    errdefer aw.deinit();
    try writePreferredMessageBase64ExecutionResultJson(&aw.writer, allocator, result);
    return try aw.toOwnedSlice();
}
...
pub fn writePreferredSignedTransactionExecutionResultJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredSignedTransactionExecutionResult,
) !void {
    var first = true;
    var diagnostics = try buildInvocationDiagnosticsFromPreferredExecutionReport(
        allocator,
    ...
    const transaction_base64 = try result.transaction.toBase64(allocator);
    ...
    try writeJsonBoolField(writer, &first, "used_fallback", result.execution_report.used_fallback);
    try writeJsonBoolField(writer, &first, "can_execute_selected_mode", result.execution_report.can_execute_selected_mode);
    try writeJsonStringField(writer, &first, "transaction_mode", @tagName(std.meta.activeTag(result.transaction)));
...
pub fn allocPreferredSignedTransactionExecutionResultJson(
    allocator: Allocator,
    result: *const PreferredSignedTransactionExecutionResult,
) ![]u8 {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    errdefer aw.deinit();
    try writePreferredSignedTransactionExecutionResultJson(&aw.writer, allocator, result);
    return try aw.toOwnedSlice();
}
...
fn writePreferredBytesExecutionResultTextWithLabel(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
    label: []const u8,
...
pub fn writePreferredMessageBytesExecutionResultText(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
) !void {
...
pub fn writePreferredMessageBase64ExecutionResultText(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredBytesExecutionResult,
) !void {
...
pub fn writePreferredSignedTransactionExecutionResultText(
    writer: *std.Io.Writer,
    allocator: Allocator,
    result: *const PreferredSignedTransactionExecutionResult,
) !void {
    ...
    try writer.print("used fallback: {}\n", .{result.execution_report.used_fallback});
    try writer.print("can execute selected mode: {}\n", .{result.execution_report.can_execute_selected_mode});
    try writer.print("transaction mode: {s}\n", .{@tagName(std.meta.activeTag(result.transaction))});
...
pub fn writeInvocationSummaryText(
    writer: *std.Io.Writer,
    allocator: Allocator,
    summary: OwnedInvocationSummary,
) !void {
    const payer_base58 = try summary.payer.toBase58(allocator);
    defer allocator.free(payer_base58);
    ...
    try writer.print("writable account count: {d}\n", .{summary.writable_account_count});
    try writer.print("readonly account count: {d}\n", .{summary.readonly_account_count});
    try writer.print("lookup table count: {d}\n", .{summary.address_lookup_table_count});

    if (summary.recent_blockhash) |value| {
        const recent_blockhash_base58 = try value.toBase58(allocator);
        defer allocator.free(recent_blockhash_base58);
        try writer.print("recent blockhash: {s}\n", .{recent_blockhash_base58});
    }
    if (summary.nonce_account) |value| {
        const nonce_account_base58 = try value.toBase58(allocator);
        defer allocator.free(nonce_account_base58);
        try writer.print("nonce account: {s}\n", .{nonce_account_base58});
    }
    if (summary.nonce_authority) |value| {
        const nonce_authority_base58 = try value.toBase58(allocator);
        defer allocator.free(nonce_authority_base58);
        try writer.print("nonce authority: {s}\n", .{nonce_authority_base58});
    }
    if (summary.signer_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "signer pubkeys", summary.signer_pubkeys);
    }
    if (summary.program_ids.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "program ids", summary.program_ids);
    }
}

pub fn writeInvocationSummaryJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    summary: OwnedInvocationSummary,
) !void {
    ...
    const payer_base58 = try summary.payer.toBase58(allocator);
    defer allocator.free(payer_base58);
    const recent_blockhash_base58 = if (summary.recent_blockhash) |value|
        try value.toBase58(allocator)
    else
        null;
    defer if (recent_blockhash_base58) |value| allocator.free(value);
    const nonce_account_base58 = if (summary.nonce_account) |value|
        try value.toBase58(allocator)
    else
        null;
    defer if (nonce_account_base58) |value| allocator.free(value);
    const nonce_authority_base58 = if (summary.nonce_authority) |value|
        try value.toBase58(allocator)
    else
        null;
    defer if (nonce_authority_base58) |value| allocator.free(value);
    ...
    try writer.writeAll("{");
    try writeJsonStringField(writer, &first, "payer", payer_base58);
    try writeJsonUsizeField(writer, &first, "instruction_count", summary.instruction_count);
    try writeJsonUsizeField(writer, &first, "account_count", summary.account_count);
    try writeJsonUsizeField(writer, &first, "signer_count", summary.signer_count);
    try writeJsonUsizeField(writer, &first, "writable_account_count", summary.writable_account_count);
    try writeJsonUsizeField(writer, &first, "readonly_account_count", summary.readonly_account_count);
    try writeJsonUsizeField(writer, &first, "lookup_table_count", summary.address_lookup_table_count);
    try writeJsonStringField(writer, &first, "recent_blockhash", recent_blockhash_base58);
    try writeJsonStringField(writer, &first, "nonce_account", nonce_account_base58);
    try writeJsonStringField(writer, &first, "nonce_authority", nonce_authority_base58);
    try writeJsonPubkeyArrayField(writer, &first, "signer_pubkeys", allocator, summary.signer_pubkeys);
    try writeJsonPubkeyArrayField(writer, &first, "program_ids", allocator, summary.program_ids);
    try writer.writeAll("}");
}

pub fn allocInvocationSummaryJson(
    allocator: Allocator,
    summary: OwnedInvocationSummary,
) ![]u8 {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    errdefer aw.deinit();
    try writeInvocationSummaryJson(&aw.writer, allocator, summary);
    return try aw.toOwnedSlice();
}

pub fn writeInvocationPlanText(
    writer: *std.Io.Writer,
    allocator: Allocator,
    plan: OwnedInvocationPlan,
) !void {
    const payer_base58 = try plan.payer.toBase58(allocator);
    defer allocator.free(payer_base58);

    try writer.print("payer: {s}\n", .{payer_base58});
    try writer.print("blockhash mode: {s}\n", .{invocationBlockhashModeJsonLabel(plan.blockhash_mode)});
    try writer.print("instruction count: {d}\n", .{plan.instruction_count});
    try writer.print("account count: {d}\n", .{plan.account_count});
    try writer.print("signer count: {d}\n", .{plan.signer_count});
    try writer.print("writable account count: {d}\n", .{plan.writable_account_count});
    try writer.print("readonly account count: {d}\n", .{plan.readonly_account_count});
    try writer.print("lookup table count: {d}\n", .{plan.address_lookup_table_count});

    if (plan.recent_blockhash) |value| {
        const recent_blockhash_base58 = try value.toBase58(allocator);
        defer allocator.free(recent_blockhash_base58);
        try writer.print("recent blockhash: {s}\n", .{recent_blockhash_base58});
    }
    if (plan.nonce_account) |value| {
        const nonce_account_base58 = try value.toBase58(allocator);
        defer allocator.free(nonce_account_base58);
        try writer.print("nonce account: {s}\n", .{nonce_account_base58});
    }
    if (plan.nonce_authority) |value| {
        const nonce_authority_base58 = try value.toBase58(allocator);
        defer allocator.free(nonce_authority_base58);
        try writer.print("nonce authority: {s}\n", .{nonce_authority_base58});
    }
    if (plan.signer_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "signer pubkeys", plan.signer_pubkeys);
    }
    if (plan.program_ids.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "program ids", plan.program_ids);
    }
    if (plan.lookup_table_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "lookup table pubkeys", plan.lookup_table_pubkeys);
    }
}

pub fn writeInvocationPlanJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    plan: OwnedInvocationPlan,
) !void {
    ...
    const payer_base58 = try plan.payer.toBase58(allocator);
    defer allocator.free(payer_base58);
    const recent_blockhash_base58 = if (plan.recent_blockhash) |value|
        try value.toBase58(allocator)
    else
        null;
    defer if (recent_blockhash_base58) |value| allocator.free(value);
    const nonce_account_base58 = if (plan.nonce_account) |value|
        try value.toBase58(allocator)
    else
        null;
    defer if (nonce_account_base58) |value| allocator.free(value);
    const nonce_authority_base58 = if (plan.nonce_authority) |value|
        try value.toBase58(allocator)
    else
        null;
    defer if (nonce_authority_base58) |value| allocator.free(value);
    ...
    try writer.writeAll("{");
    try writeJsonStringField(writer, &first, "payer", payer_base58);
    try writeJsonStringField(writer, &first, "blockhash_mode", invocationBlockhashModeJsonLabel(plan.blockhash_mode));
    try writeJsonUsizeField(writer, &first, "instruction_count", plan.instruction_count);
    try writeJsonUsizeField(writer, &first, "account_count", plan.account_count);
    try writeJsonUsizeField(writer, &first, "signer_count", plan.signer_count);
    try writeJsonUsizeField(writer, &first, "writable_account_count", plan.writable_account_count);
    try writeJsonUsizeField(writer, &first, "readonly_account_count", plan.readonly_account_count);
    try writeJsonUsizeField(writer, &first, "lookup_table_count", plan.address_lookup_table_count);
    try writeJsonStringField(writer, &first, "recent_blockhash", recent_blockhash_base58);
    try writeJsonStringField(writer, &first, "nonce_account", nonce_account_base58);
    try writeJsonStringField(writer, &first, "nonce_authority", nonce_authority_base58);
    try writeJsonPubkeyArrayField(writer, &first, "signer_pubkeys", allocator, plan.signer_pubkeys);
    try writeJsonPubkeyArrayField(writer, &first, "program_ids", allocator, plan.program_ids);
    try writeJsonPubkeyArrayField(writer, &first, "lookup_table_pubkeys", allocator, plan.lookup_table_pubkeys);
    try writer.writeAll("}");
}

pub fn allocInvocationPlanJson(
    allocator: Allocator,
    plan: OwnedInvocationPlan,
) ![]u8 {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    errdefer aw.deinit();
    try writeInvocationPlanJson(&aw.writer, allocator, plan);
    return try aw.toOwnedSlice();
}

pub fn writeInvocationLookupCoverageText(
    writer: *std.Io.Writer,
    allocator: Allocator,
    coverage: OwnedInvocationLookupCoverage,
) !void {
    try writer.print("lookup table count: {d}\n", .{coverage.lookup_table_pubkeys.len});
    try writer.print("lookup table address count: {d}\n", .{coverage.lookup_table_address_pubkeys.len});
    try writer.print("lookup candidate count: {d}\n", .{coverage.candidate_pubkeys.len});
    try writer.print("lookup covered count: {d}\n", .{coverage.covered_pubkeys.len});
    try writer.print("lookup uncovered count: {d}\n", .{coverage.uncovered_pubkeys.len});
    try writer.print("fully covered: {}\n", .{coverage.fully_covered});

    if (coverage.lookup_table_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "lookup table pubkeys", coverage.lookup_table_pubkeys);
    }
    if (coverage.lookup_table_address_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "lookup table address pubkeys", coverage.lookup_table_address_pubkeys);
    }
    if (coverage.candidate_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "lookup candidate pubkeys", coverage.candidate_pubkeys);
    }
    if (coverage.covered_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "lookup covered pubkeys", coverage.covered_pubkeys);
    }
    if (coverage.uncovered_pubkeys.len != 0) {
        try writeInvocationPubkeysText(writer, allocator, "lookup uncovered pubkeys", coverage.uncovered_pubkeys);
    }
}

pub fn writeInvocationLookupCoverageJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    coverage: OwnedInvocationLookupCoverage,
) !void {
    var first = true;
    try writer.writeAll("{");
    try writeJsonBoolField(writer, &first, "fully_covered", coverage.fully_covered);
    try writeJsonUsizeField(writer, &first, "lookup_table_count", coverage.lookup_table_pubkeys.len);
    try writeJsonUsizeField(writer, &first, "lookup_table_address_count", coverage.lookup_table_address_pubkeys.len);
    try writeJsonUsizeField(writer, &first, "candidate_count", coverage.candidate_pubkeys.len);
    try writeJsonUsizeField(writer, &first, "covered_count", coverage.covered_pubkeys.len);
    try writeJsonUsizeField(writer, &first, "uncovered_count", coverage.uncovered_pubkeys.len);
    try writeJsonPubkeyArrayField(writer, &first, "lookup_table_pubkeys", allocator, coverage.lookup_table_pubkeys);
    try writeJsonPubkeyArrayField(writer, &first, "lookup_table_address_pubkeys", allocator, coverage.lookup_table_address_pubkeys);
    try writeJsonPubkeyArrayField(writer, &first, "lookup_candidate_pubkeys", allocator, coverage.candidate_pubkeys);
    try writeJsonPubkeyArrayField(writer, &first, "lookup_covered_pubkeys", allocator, coverage.covered_pubkeys);
    try writeJsonPubkeyArrayField(writer, &first, "lookup_uncovered_pubkeys", allocator, coverage.uncovered_pubkeys);
    try writer.writeAll("}");
}

pub fn allocInvocationLookupCoverageJson(
    allocator: Allocator,
    coverage: OwnedInvocationLookupCoverage,
) ![]u8 {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    errdefer aw.deinit();
    try writeInvocationLookupCoverageJson(&aw.writer, allocator, coverage);
    return try aw.toOwnedSlice();
}

pub fn writeInvocationSummaryTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
    ...
    try writeInvocationSummaryText(writer, allocator, summary);
}

pub fn allocInvocationSummaryJsonFromInvocationSpecJson(
    allocator: Allocator,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
) ![]u8 {
    ...
    defer summary.deinit(allocator);
    return try allocInvocationSummaryJson(allocator, summary);
}

pub fn writeInvocationPlanTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
    ...
    try writeInvocationPlanText(writer, allocator, plan);
}

pub fn allocInvocationPlanJsonFromInvocationSpecJson(
    allocator: Allocator,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
) ![]u8 {
    ...
    defer plan.deinit(allocator);
    return try allocInvocationPlanJson(allocator, plan);
}

pub fn writeInvocationLookupCoverageTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
    ...
    try writeInvocationLookupCoverageText(writer, allocator, coverage);
}

pub fn allocInvocationLookupCoverageJsonFromInvocationSpecJson(
    allocator: Allocator,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
) ![]u8 {
...
pub fn writeInvocationSummaryTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    owned_spec: *const OwnedInvocationSpec,
) !void {
    var summary = try buildInvocationSummaryFromOwnedInvocationSpecRef(
        allocator,
        owned_spec,
    );
    defer summary.deinit(allocator);
...
pub fn allocInvocationSummaryJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    owned_spec: *const OwnedInvocationSpec,
) ![]u8 {
    var summary = try buildInvocationSummaryFromOwnedInvocationSpecRef(
...
pub fn writeInvocationPlanTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    owned_spec: *const OwnedInvocationSpec,
) !void {
    var plan = try buildInvocationPlanFromOwnedInvocationSpecRef(
        allocator,
        owned_spec,
    );
    defer plan.deinit(allocator);
...
pub fn allocInvocationPlanJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    owned_spec: *const OwnedInvocationSpec,
) ![]u8 {
    var plan = try buildInvocationPlanFromOwnedInvocationSpecRef(
...
pub fn writeInvocationLookupCoverageTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    owned_spec: *const OwnedInvocationSpec,
) !void {
    var coverage = try buildInvocationLookupCoverageFromOwnedInvocationSpecRef(
        allocator,
        owned_spec,
    );
    defer coverage.deinit(allocator);
...
pub fn allocInvocationLookupCoverageJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    owned_spec: *const OwnedInvocationSpec,
) ![]u8 {
    var coverage = try buildInvocationLookupCoverageFromOwnedInvocationSpecRef(
...
pub fn writeInvocationSummaryTextFromOwnedResolvedInvocationRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    resolved: *const OwnedResolvedInvocation,
) !void {
...
pub fn allocInvocationSummaryJsonFromOwnedResolvedInvocationRef(
    allocator: Allocator,
    resolved: *const OwnedResolvedInvocation,
) ![]u8 {
    var summary = try buildInvocationSummaryFromOwnedResolvedInvocationRef(
...
pub fn writeInvocationPlanTextFromOwnedResolvedInvocationRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    resolved: *const OwnedResolvedInvocation,
) !void {
    var plan = try buildInvocationPlanFromOwnedResolvedInvocationRef(
        allocator,
        resolved,
    );
    defer plan.deinit(allocator);
...
pub fn allocInvocationPlanJsonFromOwnedResolvedInvocationRef(
    allocator: Allocator,
    resolved: *const OwnedResolvedInvocation,
) ![]u8 {
    var plan = try buildInvocationPlanFromOwnedResolvedInvocationRef(
...
pub fn writeInvocationLookupCoverageTextFromOwnedResolvedInvocationRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    resolved: *const OwnedResolvedInvocation,
) !void {
    var coverage = try buildInvocationLookupCoverageFromOwnedResolvedInvocationRef(
        allocator,
        resolved,
    );
    defer coverage.deinit(allocator);
...
pub fn allocInvocationLookupCoverageJsonFromOwnedResolvedInvocationRef(
    allocator: Allocator,
    resolved: *const OwnedResolvedInvocation,
) ![]u8 {
    var coverage = try buildInvocationLookupCoverageFromOwnedResolvedInvocationRef(
...
pub fn buildPreferredInvocationModeResolution(
    mode_report: *const InvocationModeReport,
    options: PreferredInvocationModeOptions,
) PreferredInvocationModeResolution {
    const requested_mode_buildable = if (options.preferred_mode) |requested_mode|
    ...
    const selected_mode = blk: {
        if (options.preferred_mode) |requested_mode| {
            if (requested_mode_buildable) break :blk requested_mode;
            if (!options.allow_fallback) break :blk null;
        }
        break :blk mode_report.preferred_mode;
    };

    return .{
        .requested_mode = options.preferred_mode,
        .selected_mode = selected_mode,
        .requested_mode_buildable = requested_mode_buildable,
        .used_fallback = if (options.preferred_mode) |requested_mode|
...
pub fn writePreferredInvocationAnalysisTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
...
pub fn allocPreferredInvocationAnalysisJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
...
pub fn writePreferredPreparedSignedTransactionTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
...
pub fn allocPreferredPreparedSignedTransactionJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
...
pub fn writePreferredPreparedInvocationTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
...
pub fn allocPreferredPreparedInvocationJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
...
pub fn writePreferredSignedTransactionExecutionResultTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    ...
    var result = try buildPreferredSignedTransactionExecutionResultFromInvocationSpecJson(
        allocator,
        rpc,
        family,
        invocation_spec_json,
...
pub fn allocPreferredSignedTransactionExecutionResultJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
...
pub fn writePreferredMessageBytesExecutionResultTextFromInvocationSpecJson(
    ...
    var result = try buildPreferredMessageBytesExecutionResultFromInvocationSpecJson(
        allocator,
        rpc,
        family,
        invocation_spec_json,
    ...
    defer result.deinit(allocator);
    try writePreferredMessageBytesExecutionResultText(writer, allocator, &result);
}

pub fn allocPreferredMessageBytesExecutionResultJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
    ...
    defer result.deinit(allocator);
    return try allocPreferredMessageBytesExecutionResultJson(allocator, &result);
}

pub fn writePreferredMessageBase64ExecutionResultTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    ...
    var result = try buildPreferredMessageBase64ExecutionResultFromInvocationSpecJson(
        allocator,
        rpc,
        family,
        invocation_spec_json,
    ...
    defer result.deinit(allocator);
    try writePreferredMessageBase64ExecutionResultText(writer, allocator, &result);
}

pub fn allocPreferredMessageBase64ExecutionResultJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
    ...
    defer result.deinit(allocator);
    return try allocPreferredMessageBase64ExecutionResultJson(allocator, &result);
}

pub fn writePreferredOwnedMessageExecutionResultTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
...
pub fn allocPreferredOwnedMessageExecutionResultJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
...
pub fn writePreferredResolvedInvocationExecutionResultTextFromInvocationSpecJson(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
...
pub fn allocPreferredResolvedInvocationExecutionResultJsonFromInvocationSpecJson(
    allocator: Allocator,
    rpc: anytype,
    family: InvokeFamily,
    invocation_spec_json: []const u8,
...
pub fn buildPreferredInvocationModeResolutionFromOwnedInvocationSpec(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    build_options: BuildInvocationSpecOptions,
...
pub fn buildPreferredInvocationExecutionReportFromOwnedInvocationSpec(
    ...
    errdefer report.deinit(allocator);
    const mode_resolution = buildPreferredInvocationModeResolution(&mode_report, options.mode);
    ...
    return .{
        ...
        .requested_mode_buildable = mode_resolution.requested_mode_buildable,
        .used_fallback = mode_resolution.used_fallback,
        .can_execute_selected_mode = mode_resolution.selected_mode != null and report.can_execute,
    };
}
...
pub fn buildPreferredInvocationModeResolutionFromOwnedResolvedInvocationRef(
    allocator: Allocator,
    rpc: anytype,
    resolved: *const OwnedResolvedInvocation,
    build_options: BuildInvocationSpecOptions,
...
pub fn writePreferredInvocationAnalysisTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn allocPreferredInvocationAnalysisJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    options: BuildPreferredInvocationSpecOptions,
) ![]u8 {
    return try allocPreferredInvocationAnalysisJsonFromOwnedInvocationSpec(
        allocator,
        rpc,
        owned_spec,
        options,
...
pub fn writePreferredPreparedInvocationTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    ...
    try writePreferredPreparedInvocationTextFromOwnedInvocationSpec(
        writer,
        allocator,
        rpc,
        owned_spec,
...
pub fn allocPreferredPreparedInvocationJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    options: BuildPreferredInvocationSpecOptions,
) ![]u8 {
    return try allocPreferredPreparedInvocationJsonFromOwnedInvocationSpec(
        allocator,
        rpc,
        owned_spec,
        options,
...
pub fn writePreferredPreparedSignedTransactionTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn allocPreferredPreparedSignedTransactionJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    options: BuildPreferredInvocationSpecOptions,
) ![]u8 {
    return try allocPreferredPreparedSignedTransactionJsonFromOwnedInvocationSpec(
        allocator,
        rpc,
        owned_spec,
        options,
...
pub fn allocSentPreparedInvocationJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    rpc: anytype,
    versioned: bool,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn allocSimulatedPreparedInvocationJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    rpc: anytype,
    versioned: bool,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn writePreparedInvocationFeeTextFromOwnedInvocationSpecRef(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    versioned: bool,
...
pub fn allocPreparedInvocationFeeJsonFromOwnedInvocationSpecRef(
    allocator: Allocator,
    rpc: anytype,
    versioned: bool,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn writePreferredSignedTransactionExecutionResultTextFromOwnedInvocationSpec(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn allocPreferredSignedTransactionExecutionResultJsonFromOwnedInvocationSpec(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    options: BuildPreferredInvocationSpecOptions,
...
pub fn allocPreferredMessageBytesExecutionResultJsonFromOwnedInvocationSpec(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    options: BuildPreferredInvocationSpecOptions,
...
pub fn writePreferredMessageBase64ExecutionResultTextFromOwnedInvocationSpec(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
...
pub fn allocPreferredMessageBase64ExecutionResultJsonFromOwnedInvocationSpec(
    allocator: Allocator,
    rpc: anytype,
    owned_spec: *const OwnedInvocationSpec,
    options: BuildPreferredInvocationSpecOptions,
...
pub fn writePreferredMessageBytesExecutionResultTextFromOwnedResolvedInvocation(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    resolved: *const OwnedResolvedInvocation,
...
pub fn allocPreferredMessageBytesExecutionResultJsonFromOwnedResolvedInvocation(
    allocator: Allocator,
    rpc: anytype,
    resolved: *const OwnedResolvedInvocation,
    options: BuildPreferredInvocationSpecOptions,
...
pub fn writePreferredMessageBase64ExecutionResultTextFromOwnedResolvedInvocation(
    writer: *std.Io.Writer,
    allocator: Allocator,
    rpc: anytype,
    resolved: *const OwnedResolvedInvocation,
...
pub fn allocPreferredMessageBase64ExecutionResultJsonFromOwnedResolvedInvocation(
    allocator: Allocator,
    rpc: anytype,
    resolved: *const OwnedResolvedInvocation,
    options: BuildPreferredInvocationSpecOptions,
...
pub fn sendTransactionFromInvocationSpecJson(
    ...
    defer owned_spec.deinit(allocator);
    return try sendTransactionFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        versioned,
        &owned_spec,
...
pub fn sendAndConfirmInvocationSpecJson(
    ...
    defer owned_spec.deinit(allocator);
    return try sendAndConfirmInvocationFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        versioned,
        &owned_spec,
...
pub fn simulateTransactionFromInvocationSpecJson(
    ...
    defer owned_spec.deinit(allocator);
    return try simulateTransactionFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        versioned,
        &owned_spec,
...
pub fn sendAndConfirmInvocationSpecJsonWithSpinnerOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try sendAndConfirmInvocationFromOwnedInvocationSpecWithSpinnerOptions(
        allocator,
        rpc,
        versioned,
        &owned_spec,
...
pub fn getFeeForInvocationSpecJson(
    ...
    defer owned_spec.deinit(allocator);
    return try getFeeForInvocationSpecFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        versioned,
        &owned_spec,
...
pub fn buildLegacyTransactionBase64FromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildLegacyTransactionBase64FromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildVersionedTransactionBase64FromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildVersionedTransactionBase64FromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildOwnedLegacyMessageFromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildOwnedLegacyMessageFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildLegacyMessageBytesFromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildLegacyMessageBytesFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildLegacyMessageBase64FromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildLegacyMessageBase64FromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildSignedLegacyTransactionFromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildSignedLegacyTransactionFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildOwnedVersionedMessageFromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildOwnedVersionedMessageFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildVersionedMessageBytesFromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildVersionedMessageBytesFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildVersionedMessageBase64FromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildVersionedMessageBase64FromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
pub fn buildSignedVersionedTransactionFromInvocationSpecJsonWithOptions(
    ...
    defer owned_spec.deinit(allocator);
    return try buildSignedVersionedTransactionFromOwnedInvocationSpecWithOptions(
        allocator,
        rpc,
        &owned_spec,
        options,
...
test "invoke.allocInvocationSummaryJsonFromInvocationSpecJson emits summary fields" {
    const allocator = std.testing.allocator;

    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 491, 492, 493);
    defer allocator.free(spec_json);

    const json = try allocInvocationSummaryJsonFromInvocationSpecJson(
        allocator,
        .instructions,
        spec_json,
    );
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"instruction_count\":1") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"signer_pubkeys\":[") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"program_ids\":[") != null);
}

test "invoke.writeInvocationPlanTextFromOwnedResolvedInvocationRef emits planning fields" {
    const allocator = std.testing.allocator;

    const spec_json = try allocProgramInvocationSpecJsonWithLookupTable(allocator, 494, 495, 496, 497, 498);
    defer allocator.free(spec_json);
    ...
    try writeInvocationPlanTextFromOwnedResolvedInvocationRef(
        &aw.writer,
        allocator,
        &resolved,
    );
    ...
    try std.testing.expect(std.mem.indexOf(u8, text, "blockhash mode: explicit_recent_blockhash") != null);
    try std.testing.expect(std.mem.indexOf(u8, text, "lookup table pubkeys") != null);
}

test "invoke.allocInvocationLookupCoverageJsonFromInvocationSpecJson emits lookup fields" {
    const allocator = std.testing.allocator;

    const spec_json = try allocProgramInvocationSpecJsonWithLookupTable(allocator, 499, 500, 501, 502, 503);
    defer allocator.free(spec_json);

    const json = try allocInvocationLookupCoverageJsonFromInvocationSpecJson(
        allocator,
        .program,
        spec_json,
    );
...
test "invoke.buildLegacyMessageBase64FromInvocationSpecJsonWithOptions matches typed owned spec builder" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 24, 25, 26);
    defer allocator.free(spec_json);
    ...
    const expected = try buildLegacyMessageBase64FromOwnedInvocationSpecWithOptions(
        allocator,
        DummyRpc{},
        &owned_spec,
        .{ .blockhash_commitment = .processed },
    ...
    const actual = try buildLegacyMessageBase64FromInvocationSpecJsonWithOptions(
        allocator,
        DummyRpc{},
        .instructions,
        spec_json,
...
test "invoke.getFeeForMessageFromInvocationSpecJson uses typed fee path" {
    const allocator = std.testing.allocator;
...
test "invoke.sendTransactionFromInvocationSpecJson uses typed send path" {
    const allocator = std.testing.allocator;
    ...
    const program_id_base58 = try program_id.toBase58(allocator);
    defer allocator.free(program_id_base58);
    const recent_blockhash = sdk.Hash.fromBytes(.{208} ** 32);
    const recent_blockhash_base58 = try recent_blockhash.toBase58(allocator);
    defer allocator.free(recent_blockhash_base58);
    ...
    try std.testing.expectEqualStrings("invoke-typed-send", signature);
    try std.testing.expect(rpc.captured_skip_preflight);
}

test "invoke.sendAndConfirmInvocationSpecJson uses typed confirm path" {
    const allocator = std.testing.allocator;
    const idl_json =
        \\{
        \\  "address": "11111111111111111111111111111111",
    ...
    const authority = try client.Keypair.fromSecretKeyBytes(authority_raw.secret_key.toBytes());
    const recent_blockhash = [_]u8{0xD3} ** 32;
    const recent_blockhash_base58 = try client.encodeBase58(allocator, &recent_blockhash);
    defer allocator.free(recent_blockhash_base58);
    ...
    try std.testing.expectEqualStrings("invoke-typed-confirm", signature);
    try std.testing.expectEqual(rpc_types.Commitment.confirmed, rpc.captured_commitment.?);
}

test "invoke.simulateTransactionFromInvocationSpecJson uses typed simulate path" {
    const allocator = std.testing.allocator;
    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 211, 212, 213);
    defer allocator.free(spec_json);
...
test "invoke.sendAndConfirmInvocationSpecJsonWithSpinnerOptions uses typed spinner path" {
    const allocator = std.testing.allocator;
    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 213, 214, 215);
    defer allocator.free(spec_json);
...
test "invoke.getFeeForInvocationSpecJson uses typed fee path" {
    const allocator = std.testing.allocator;
...
test "invoke.allocPreferredInvocationAnalysisJsonFromInvocationSpecJson emits reusable analysis fields" {
    ...
    const json = try allocPreferredInvocationAnalysisJsonFromInvocationSpecJson(
        allocator,
        DummyRpc{},
        .program,
        spec_json,
...
test "invoke.allocPreferredSignedTransactionExecutionResultJsonFromOwnedInvocationSpec emits transaction fields" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocProgramInvocationSpecJsonWithLookupTable(allocator, 698, 699, 700, 701, 702);
    defer allocator.free(spec_json);
    ...
    const json = try allocPreferredSignedTransactionExecutionResultJsonFromOwnedInvocationSpec(
        allocator,
        DummyRpc{},
        &owned_spec,
        .{
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"requested_mode\":\"legacy\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"selected_mode\":\"versioned\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"transaction_mode\":\"versioned\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"transaction_base64\":\"") != null);
}
...
test "invoke.allocPreferredMessageBytesExecutionResultJsonFromOwnedInvocationSpec emits encoded bytes" {
    ...
    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 706, 707, 708);
    defer allocator.free(spec_json);
    ...
    const json = try allocPreferredMessageBytesExecutionResultJsonFromOwnedInvocationSpec(
        allocator,
        DummyRpc{},
        &owned_spec,
        .{},
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"selected_mode\":\"legacy\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"message_bytes_base64\":\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"diagnostic_error_count\":0") != null);
}

test "invoke.writePreferredSignedTransactionExecutionResultTextFromOwnedInvocationSpec emits text" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 703, 704, 705);
    defer allocator.free(spec_json);
    ...
    try writePreferredSignedTransactionExecutionResultTextFromOwnedInvocationSpec(
        &aw.writer,
        allocator,
        DummyRpc{},
        &owned_spec,
...
test "invoke.writePreferredMessageBase64ExecutionResultTextFromOwnedResolvedInvocation emits text" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocProgramInvocationSpecJsonWithLookupTable(allocator, 709, 710, 711, 712, 713);
    defer allocator.free(spec_json);
    ...
    try writePreferredMessageBase64ExecutionResultTextFromOwnedResolvedInvocation(
        &aw.writer,
        allocator,
        DummyRpc{},
        &resolved,
    ...
    try std.testing.expect(std.mem.indexOf(u8, text, "selected mode: versioned") != null);
    try std.testing.expect(std.mem.indexOf(u8, text, "used fallback: true") != null);
    try std.testing.expect(std.mem.indexOf(u8, text, "message base64: ") != null);
}
...
test "invoke.allocPreferredMessageBytesExecutionResultJsonFromInvocationSpecJson emits encoded bytes" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 413, 414, 415);
    defer allocator.free(spec_json);

    const json = try allocPreferredMessageBytesExecutionResultJsonFromInvocationSpecJson(
        allocator,
        DummyRpc{},
        .instructions,
        spec_json,
...
test "invoke.writePreferredSignedTransactionExecutionResultTextFromInvocationSpecJson emits text" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 416, 417, 418);
    defer allocator.free(spec_json);

    var aw: std.Io.Writer.Allocating = .init(allocator);
    defer aw.deinit();

    try writePreferredSignedTransactionExecutionResultTextFromInvocationSpecJson(
        &aw.writer,
        allocator,
        DummyRpc{},
        .instructions,
...
test "invoke.allocPreferredResolvedInvocationExecutionResultJsonFromInvocationSpecJson emits resolved fields" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocProgramInvocationSpecJsonWithLookupTable(allocator, 419, 420, 421, 422, 423);
    defer allocator.free(spec_json);

    const json = try allocPreferredResolvedInvocationExecutionResultJsonFromInvocationSpecJson(
        allocator,
        DummyRpc{},
        .program,
        spec_json,
...
test "invoke.allocSentPreparedInvocationJsonFromOwnedInvocationSpecRef emits signature fields" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {
        pub fn sendTransactionTyped(
            self: *@This(),
            ...
            _ = transaction;
            _ = options;
            return try allocator.dupe(u8, "typed-explicit-send-ref");
        }
    ...
    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 250, 251, 252);
    defer allocator.free(spec_json);
    ...
    const json = try allocSentPreparedInvocationJsonFromOwnedInvocationSpecRef(
        allocator,
        DummyRpc{},
        false,
        &owned_spec,
...
test "invoke.writePreparedInvocationFeeTextFromOwnedInvocationSpecRef emits fee text" {
    ...
    const spec_json = try allocMinimalInstructionsInvocationSpecJson(allocator, 253, 254, 255);
    defer allocator.free(spec_json);
    ...
    try writePreparedInvocationFeeTextFromOwnedInvocationSpecRef(
        &aw.writer,
        allocator,
        DummyRpc{},
        false,
...
test "invoke.buildPreparedInvocationFromInvocationSpecJsonWithOptions matches typed owned spec builder" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};

    const spec_json = try allocProgramInvocationSpecJsonWithLookupTable(allocator, 116, 117, 118, 119, 120);
    defer allocator.free(spec_json);

    var from_json = try buildPreparedInvocationFromInvocationSpecJsonWithOptions(
        allocator,
        DummyRpc{},
        .program,
        true,
    ...
    var from_typed = try buildPreparedInvocationFromOwnedInvocationSpecWithOptions(
        allocator,
        DummyRpc{},
        true,
        try buildOwnedInvocationSpecFromInvocationSpecJson(
    ...
    defer from_typed.deinit(allocator);

    const from_json_encoded = try allocPreparedInvocationJson(allocator, &from_json);
    defer allocator.free(from_json_encoded);
    const from_typed_encoded = try allocPreparedInvocationJson(allocator, &from_typed);
    defer allocator.free(from_typed_encoded);

    try std.testing.expectEqualStrings(from_typed_encoded, from_json_encoded);
}
...
test "invoke.allocPreferredPreparedInvocationJsonFromInvocationSpecJson preserves fallback fields" {
    const allocator = std.testing.allocator;
    const DummyRpc = struct {};
    ...
    const json = try allocPreferredPreparedInvocationJsonFromInvocationSpecJson(
        allocator,
        DummyRpc{},
        .program,
        spec_json,
