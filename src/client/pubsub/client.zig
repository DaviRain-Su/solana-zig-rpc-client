...
const SignatureNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runSignatureNotificationCallback(context: *SignatureNotificationCallbackState) void {
    context.run();
}

const RootNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runRootNotificationCallback(context: *RootNotificationCallbackState) void {
    context.run();
}

const VoteNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runVoteNotificationCallback(context: *VoteNotificationCallbackState) void {
    context.run();
}

const AccountNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runAccountNotificationCallback(context: *AccountNotificationCallbackState) void {
    context.run();
}

const LogsNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runLogsNotificationCallback(context: *LogsNotificationCallbackState) void {
    context.run();
}

const ProgramNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runProgramNotificationCallback(context: *ProgramNotificationCallbackState) void {
    context.run();
}

const SlotNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runSlotNotificationCallback(context: *SlotNotificationCallbackState) void {
    context.run();
}

const BlockNotificationCallbackState = struct {
    subscription: *PubsubSubscription,
    context: ?*anyopaque,
...
fn runBlockNotificationCallback(context: *BlockNotificationCallbackState) void {
    context.run();
}
...
pub const PubsubRootSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *RootNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubVoteSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *VoteNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubBlockSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *BlockNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubSignatureSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *SignatureNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubAccountSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *AccountNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubLogsSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *LogsNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubProgramSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *ProgramNotificationCallbackState,
    callback_thread: std.Thread,
...
pub const PubsubSlotSubscriptionWithCallback = struct {
    subscription: *PubsubSubscription,
    callback_state: *SlotNotificationCallbackState,
    callback_thread: std.Thread,
...
const State = struct {
    ...
    fn heartbeatLoop(self: *State) void {
        while (true) {
            self.mutex.lock();
            const interval_ms = self.options.heartbeat_interval_ms orelse {
                self.heartbeat_thread = null;
                self.mutex.unlock();
                return;
            };
            ...
            if (self.options.heartbeat_interval_ms == null) {
                self.heartbeat_thread = null;
                self.mutex.unlock();
                return;
            }
...
pub const PubsubClient = struct {
    ...
    pub fn canReconnect(self: *const Self) bool {
        return self.isAutoReconnectEnabled() and self.hasRemainingReconnectAttempts();
    }

    pub fn cannotReconnect(self: *const Self) bool {
    ...
    pub fn isQueueOverflowDropOldest(self: *const Self) bool {
        return self.getQueueOverflowPolicy() == .drop_oldest;
    }

    pub fn isQueueOverflowDropNewest(self: *const Self) bool {
        return self.getQueueOverflowPolicy() == .drop_newest;
    }

    pub fn isQueueOverflowCloseSubscription(self: *const Self) bool {
    ...
    pub fn setAutoReconnectEnabled(self: *Self, enabled: bool) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.auto_reconnect = enabled;
    }

    pub fn setHeartbeatIntervalMs(self: *Self, interval_ms: ?u32) !void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();

        if (self.state.shutdown_started or self.state.closed) {
            return error.Unavailable;
        }

        self.state.options.heartbeat_interval_ms = interval_ms;
        self.state.cond.broadcast();

        if (interval_ms != null and self.state.heartbeat_thread == null) {
            self.state.heartbeat_thread = try std.Thread.spawn(.{}, State.heartbeatLoopThread, .{self.state});
        }
    }

    pub fn setHeartbeatTimeoutMs(self: *Self, timeout_ms: ?u32) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.heartbeat_timeout_ms = timeout_ms;
    }

    pub fn setReconnectDelayMs(self: *Self, delay_ms: u32) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.reconnect_delay_ms = delay_ms;
    }

    pub fn setReconnectBackoffFactor(self: *Self, factor: u8) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.reconnect_backoff_factor = factor;
    }

    pub fn setReconnectMaxDelayMs(self: *Self, max_delay_ms: ?u32) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.reconnect_max_delay_ms = max_delay_ms;
    }

    pub fn setReconnectMaxAttempts(self: *Self, max_attempts: ?u32) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.reconnect_max_attempts = max_attempts;
    }

    pub fn setSubscriptionQueueLimit(self: *Self, limit: usize) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.subscription_queue_limit = limit;
    }

    pub fn setQueueOverflowPolicy(self: *Self, policy: pubsub_types.PubsubQueueOverflowPolicy) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.queue_overflow_policy = policy;
    }

    pub fn setHandshakeTimeoutMs(self: *Self, timeout_ms: u32) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.handshake_timeout_ms = timeout_ms;
    }

    pub fn setWriteTimeoutMs(self: *Self, timeout_ms: ?u32) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.write_timeout_ms = timeout_ms;
    }

    pub fn setMaxMessageSize(self: *Self, max_message_size: usize) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.max_message_size = max_message_size;
    }

    pub fn setBufferSize(self: *Self, buffer_size: usize) void {
        self.state.mutex.lock();
        defer self.state.mutex.unlock();
        self.state.options.buffer_size = buffer_size;
    }
    ...
    pub fn signatureSubscribeWithCallback(
        self: *Self,
        signature: []const u8,
        options: SignatureSubscribeOptions,
        context: ?*anyopaque,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runSignatureNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
    ...
    pub fn accountSubscribeWithCallback(
        self: *Self,
        account: []const u8,
        options: AccountSubscribeOptions,
        context: ?*anyopaque,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runAccountNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
    ...
    pub fn logsSubscribeWithCallback(
        self: *Self,
        filter: LogsSubscribeFilter,
        options: LogsSubscribeOptions,
        context: ?*anyopaque,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runLogsNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
    ...
    pub fn programSubscribeWithCallback(
        self: *Self,
        program_id: []const u8,
        options: ProgramSubscribeOptions,
        context: ?*anyopaque,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runProgramNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
    ...
    pub fn slotSubscribeWithCallback(
        self: *Self,
        context: ?*anyopaque,
        callback: SlotNotificationCallback,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runSlotNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
    ...
    pub fn slotSubscribeWithOptionsWithCallback(
        self: *Self,
        options: SlotSubscribeOptions,
        context: ?*anyopaque,
        callback: SlotNotificationCallback,
    ...
    pub fn rootSubscribeWithCallback(
        self: *Self,
        context: ?*anyopaque,
        callback: RootNotificationCallback,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runRootNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
    ...
    pub fn voteSubscribeWithCallback(
        self: *Self,
        context: ?*anyopaque,
        callback: VoteNotificationCallback,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runVoteNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
    ...
    pub fn blockSubscribeWithCallback(
        self: *Self,
        filter: BlockSubscribeFilter,
        options: BlockSubscribeOptions,
        context: ?*anyopaque,
        ...
        const callback_thread = try std.Thread.spawn(.{}, runBlockNotificationCallback, .{callback_state});
        return .{
            .subscription = subscription,
            .callback_state = callback_state,
            .callback_thread = callback_thread,
