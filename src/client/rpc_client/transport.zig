...
fn configureSocketTimeout(handle: std.net.Stream.Handle, timeout_ms: u64, option: u32) !void {
    ...
    timeout.sec = std.math.cast(@TypeOf(timeout.sec), timeout_ms / std.time.ms_per_s) orelse return error.TimeoutTooBig;
    timeout.usec = std.math.cast(@TypeOf(timeout.usec), (timeout_ms % std.time.ms_per_s) * std.time.us_per_ms) orelse return error.TimeoutTooBig;
    try std.posix.setsockopt(handle, std.posix.SOL.SOCKET, option, std.mem.asBytes(&timeout));
}
