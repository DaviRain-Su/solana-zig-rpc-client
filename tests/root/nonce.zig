...
test "root.sendAndConfirmVersionedInstructionsWithBlockhashQueryWithSpinner supports fixed blockhashes" {
    const allocator = std.testing.allocator;
    ...
    var expected_signed = try client.buildSignedVersionedTransactionV0(
        allocator,
        payer.public_key,
        Hash.fromBytes(recent_blockhash),
        instructions[0..],
    ...
    defer rpc.deinit();
    try rpc.pushMockSendAndSignatureStatusPollFlow(
        "SigVersionedSpinner111111111111111111111111111111111111111111111111111111",
        &.{
            .{ .context_slot = 110, .status = null },
            .{ .context_slot = 111, .status = .{
                .slot = 111,
                .confirmations = 1,
                .confirmation_status = "processed",
                .has_error = false,
            } },
            .{ .context_slot = 112, .status = .{
                .slot = 112,
                .confirmations = 2,
                .confirmation_status = "confirmed",
                .has_error = false,
    ...
    const signature = try rpc.sendAndConfirmVersionedInstructionsWithBlockhashQueryWithSpinner(
        payer.public_key,
        instructions[0..],
        &.{},
        &.{payer},
    ...
    try std.testing.expectEqualStrings(
        \\sending transaction...
        \\submitted transaction: SigVersionedSpinner111111111111111111111111111111111111111111111111111111
        \\waiting for transaction to be observed: SigVersionedSpinner111111111111111111111111111111111111111111111111111111
        \\waiting for confirmed confirmation: SigVersionedSpinner111111111111111111111111111111111111111111111111111111
        \\transaction confirmed: SigVersionedSpinner111111111111111111111111111111111111111111111111111111
        \\
    , captured);
}
