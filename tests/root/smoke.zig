...
test "root.newMockRequestSender aliases newWithMockRequestSender" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestSender(allocator, &.{});
    defer rpc.deinit();
...
test "root.newMockRequestSenderAndRequestSenderOptions aliases newWithMockRequestSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestSenderAndRequestSenderOptions(
        allocator,
        &.{},
        .{
            .endpoint = "custom://short-mock-request-sender",
            .commitment = .confirmed,
            .request_timeout_ms = 4_256,
            .confirm_transaction_initial_timeout_ms = 7_256,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://short-mock-request-sender", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 4_256), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 7_256), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
}

test "root.newMockRequestResponses aliases newMockRequestSender" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestResponses(allocator, &.{});
    defer rpc.deinit();
...
test "root.newMockRequestResponsesAndRequestSenderOptions aliases newMockRequestSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestResponsesAndRequestSenderOptions(
        allocator,
        &.{},
        .{
            .endpoint = "custom://short-mock-request-responses",
            .commitment = .processed,
            .request_timeout_ms = 4_266,
            .confirm_transaction_initial_timeout_ms = 7_266,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://short-mock-request-responses", rpc.url());
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 4_266), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 7_266), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
}
...
test "root.newMockRequestSenderAndCommitmentAndTimeouts preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestSenderAndCommitmentAndTimeouts(
        allocator,
        &.{},
        .confirmed,
        4_650,
    ...
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 4_650), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 7_650), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
}

test "root.newMockRequestResponsesAndCommitmentAndTimeouts preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestResponsesAndCommitmentAndTimeouts(
        allocator,
        &.{},
        .finalized,
        4_660,
    ...
    try std.testing.expectEqual(client.Commitment.finalized, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 4_660), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 7_660), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
}
...
test "root.newMockRequestSenderAndTimeoutsAndCommitment preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestSenderAndTimeoutsAndCommitment(
        allocator,
        &.{},
        4_750,
        7_750,
    ...
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 4_750), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 7_750), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
}

test "root.newMockRequestResponsesAndTimeoutsAndCommitment preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.newMockRequestResponsesAndTimeoutsAndCommitment(
        allocator,
        &.{},
        4_760,
        7_760,
    ...
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 4_760), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 7_760), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
}
...
test "root.newMockRequestSenderWithSenderAndCommitmentAndTimeouts preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(804);

    var rpc = try client.RpcClient.newMockRequestSenderWithSenderAndCommitmentAndTimeouts(
        allocator,
        sender,
        .confirmed,
        5_050,
    ...
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 5_050), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 8_050), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());

    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 804), slot);
}
...
test "root.newMockRequestSenderWithSenderAndTimeoutsAndCommitment preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(805);

    var rpc = try client.RpcClient.newMockRequestSenderWithSenderAndTimeoutsAndCommitment(
        allocator,
        sender,
        5_120,
        8_120,
    ...
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 5_120), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 8_120), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());

    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 805), slot);
}
...
test "root.newMockRequestSenderWithSender aliases newWithMockRequestSenderWithSender" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(806);

    var rpc = try client.RpcClient.newMockRequestSenderWithSender(allocator, sender);
    defer rpc.deinit();
    ...
    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 806), slot);
}

test "root.newMockRequestSenderWithSenderAndRequestSenderOptions aliases newWithMockRequestSenderWithSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(807);

    var rpc = try client.RpcClient.newMockRequestSenderWithSenderAndRequestSenderOptions(
        allocator,
        sender,
        .{
            .endpoint = "custom://short-mock-request-sender-with-sender",
            .commitment = .confirmed,
            .request_timeout_ms = 5_160,
            .confirm_transaction_initial_timeout_ms = 8_160,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://short-mock-request-sender-with-sender", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 5_160), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 8_160), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());

    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 807), slot);
}

test "root.newMockRequestMockSender aliases newMockRequestSenderWithSender" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(808);

    var rpc = try client.RpcClient.newMockRequestMockSender(allocator, sender);
    defer rpc.deinit();
    ...
    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 808), slot);
}

test "root.newMockRequestMockSenderAndRequestSenderOptions aliases newMockRequestSenderWithSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(809);

    var rpc = try client.RpcClient.newMockRequestMockSenderAndRequestSenderOptions(
        allocator,
        sender,
        .{
            .endpoint = "custom://short-mock-request-mock-sender",
            .commitment = .confirmed,
            .request_timeout_ms = 5_170,
            .confirm_transaction_initial_timeout_ms = 8_170,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://short-mock-request-mock-sender", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 5_170), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 8_170), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());

    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 809), slot);
}

test "root.newMockRequestMockSenderAndCommitmentAndTimeouts preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(810);

    var rpc = try client.RpcClient.newMockRequestMockSenderAndCommitmentAndTimeouts(
        allocator,
        sender,
        .finalized,
        5_180,
    ...
    try std.testing.expectEqual(client.Commitment.finalized, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 5_180), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 8_180), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());

    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 810), slot);
}

test "root.newMockRequestMockSenderAndTimeoutsAndCommitment preserves short alias semantics" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(811);

    var rpc = try client.RpcClient.newMockRequestMockSenderAndTimeoutsAndCommitment(
        allocator,
        sender,
        5_190,
        8_190,
    ...
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 5_190), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 8_190), rpc.getConfirmTransactionInitialTimeoutMs());
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());

    const slot = try rpc.getSlot(.processed);
    try std.testing.expectEqual(@as(u64, 811), slot);
}
...
test "root.newRequestCallback aliases newWithRequestCallback" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1715 };

    var rpc = try client.RpcClient.newRequestCallback(
        allocator,
        &context,
        customRequestSender,
    );
    ...
    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 1716), slot);
}

test "root.newRequestCallbackAndRequestSenderOptions aliases newWithRequestCallbackAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1717 };

    var rpc = try client.RpcClient.newRequestCallbackAndRequestSenderOptions(
        allocator,
        &context,
        customRequestSender,
        .{
            .endpoint = "custom://request-callback-short-alias",
            .commitment = .finalized,
            .request_timeout_ms = 24,
            .confirm_transaction_initial_timeout_ms = 37,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://request-callback-short-alias", rpc.url());
    try std.testing.expectEqual(client.Commitment.finalized, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 24), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 37), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 1718), slot);
    try std.testing.expect(context.saw_finalized_commitment);
}
...
test "root.newRequestCallbackAndDeinitAndRequestSenderOptions aliases newWithRequestCallbackAndDeinitAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1725 };

    {
        var rpc = try client.RpcClient.newRequestCallbackAndDeinitAndRequestSenderOptions(
            allocator,
            &context,
            customRequestSender,
            customRequestSenderDeinit,
            .{
                ...
                .confirm_transaction_initial_timeout_ms = 38,
            },
        );
        defer rpc.deinit();

        try std.testing.expectEqualStrings("custom://request-callback-deinit-short-alias", rpc.url());
        try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
        try std.testing.expectEqual(@as(?u64, 25), rpc.getRequestTimeoutMs());
        try std.testing.expectEqual(@as(?u64, 38), rpc.getConfirmTransactionInitialTimeoutMs());

        const slot = try rpc.getSlot(null);
        try std.testing.expectEqual(@as(u64, 1726), slot);
        try std.testing.expect(context.saw_confirmed_commitment);
    }
...
test "root.newRequestCallbackAndCommitmentAndTimeouts preserves short alias semantics" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{
        .base_slot = 1285,
    };
    ...
    {
        var rpc = try client.RpcClient.newRequestCallbackAndCommitmentAndTimeouts(
            allocator,
            &context,
            customRequestSender,
            .confirmed,
        ...
        const slot = try rpc.getSlot(null);
        try std.testing.expectEqual(@as(u64, 1286), slot);
        try std.testing.expect(context.saw_confirmed_commitment);
    }
}
...
test "root.newRequestCallbackAndDeinitAndCommitmentAndTimeouts preserves short alias semantics" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{
        .base_slot = 1295,
    };
    ...
    {
        var rpc = try client.RpcClient.newRequestCallbackAndDeinitAndCommitmentAndTimeouts(
            allocator,
            &context,
            customRequestSender,
            customRequestSenderDeinit,
        ...
        try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
        try std.testing.expectEqual(@as(?u64, 6_760), rpc.getRequestTimeoutMs());
        try std.testing.expectEqual(@as(?u64, 9_760), rpc.getConfirmTransactionInitialTimeoutMs());

        const slot = try rpc.getSlot(null);
        try std.testing.expectEqual(@as(u64, 1296), slot);
        try std.testing.expect(context.saw_confirmed_commitment);
    }
...
test "root.replaceSender aliases replaceRequestSender" {
    const allocator = std.testing.allocator;
    var first_context = RequestSenderContext{ .base_slot = 930 };
    var second_context = RequestSenderContext{ .base_slot = 940 };
    ...
    const slot = try rpc.getSlot(.confirmed);
    try std.testing.expectEqual(@as(u64, 941), slot);
    try std.testing.expectEqual(@as(usize, 1), rpc.getTransportStats().request_count);
    try std.testing.expect(second_context.saw_confirmed_commitment);
}

test "root.replaceSenderAndRequestSenderOptions aliases replaceRequestSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var first_context = RequestSenderContext{ .base_slot = 950 };
    var second_context = RequestSenderContext{ .base_slot = 960 };

    var rpc = try client.RpcClient.newWithRequestSenderAndOptions(
        allocator,
        client.RequestSender.initWithDeinit(&first_context, customRequestSender, customRequestSenderDeinit),
        .{
            .endpoint = "custom://replace-sender-first",
            .commitment = .processed,
            .request_timeout_ms = 71,
            .confirm_transaction_initial_timeout_ms = 81,
        },
    );
    defer rpc.deinit();
    ...
    try rpc.replaceSenderAndRequestSenderOptions(
        client.RequestSender.initWithDeinit(&second_context, customRequestSender, customRequestSenderDeinit),
        .{
            .endpoint = "custom://replace-sender-second",
            .commitment = .finalized,
            .request_timeout_ms = 72,
            .confirm_transaction_initial_timeout_ms = 82,
        },
    );

    try std.testing.expectEqual(@as(usize, 1), first_context.deinit_count);
    try std.testing.expectEqualStrings("custom://replace-sender-second", rpc.url());
    try std.testing.expectEqual(client.Commitment.finalized, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 72), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 82), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 961), slot);
    try std.testing.expect(second_context.saw_finalized_commitment);
}
...
test "root.replaceBorrowedMock aliases replaceWithBorrowedMockSender" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1512 };
    var sender = client.MockSender.init(allocator);
    defer sender.deinit();
    try sender.pushSlotResult(1712);
    ...
    try rpc.replaceBorrowedMock(&sender);

    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expect(rpc.isMock());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 1712), slot);
}
...
test "root.replaceWithMockResponses aliases replaceWithMock" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1813 };
    ...
    try rpc.replaceWithMockResponses(&.{});
    try rpc.pushMockSlotResult(1814);
...
test "root.replaceWithMockResponsesAndRequestSenderOptions aliases replaceWithMockAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1817 };
    ...
    try rpc.replaceWithMockResponsesAndRequestSenderOptions(
        &.{ .{ .result_json = "1815" } },
        .{
            .endpoint = "custom://replace-mock-responses",
            .commitment = .finalized,
    ...
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://replace-mock-responses", rpc.url());
    try std.testing.expectEqual(client.Commitment.finalized, rpc.getDefaultCommitment().?);
    ...
    const slot = try rpc.getSlot(.finalized);
    try std.testing.expectEqual(@as(u64, 1815), slot);
}
...
test "root.replaceMockRequestResponses aliases replaceWithMockRequestSender" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1832 };
    ...
    try rpc.replaceMockRequestResponses(&.{});
    try rpc.pushMockSlotResult(1833);
    ...
    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 1833), slot);
}
...
test "root.replaceMockRequestResponsesAndRequestSenderOptions aliases replaceWithMockRequestSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1847 };
    ...
    try rpc.replaceMockRequestResponsesAndRequestSenderOptions(
        &.{ .{ .result_json = "1834" } },
        .{
            .endpoint = "custom://replace-mock-request-responses",
            .commitment = .confirmed,
            .request_timeout_ms = 46,
            .confirm_transaction_initial_timeout_ms = 66,
        },
    ...
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://replace-mock-request-responses", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 46), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 66), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(.confirmed);
    try std.testing.expectEqual(@as(u64, 1834), slot);
}
...
test "root.replaceMockRequestMockSender aliases replaceWithMockRequestSenderWithSender" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1842 };

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(1848);
    ...
    try rpc.replaceMockRequestMockSender(sender);
    ...
    const slot = try rpc.getSlot(.confirmed);
    try std.testing.expectEqual(@as(u64, 1848), slot);
}

test "root.replaceMockRequestMockSenderAndRequestSenderOptions aliases replaceWithMockRequestSenderWithSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1843 };

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(1849);
    ...
    try rpc.replaceMockRequestMockSenderAndRequestSenderOptions(
        sender,
        .{
            .endpoint = "custom://replace-mock-request-mock-sender",
            .commitment = .confirmed,
    ...
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://replace-mock-request-mock-sender", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 48), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 68), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(.confirmed);
    try std.testing.expectEqual(@as(u64, 1849), slot);
}
...
test "root.setSender aliases setRequestSender" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 2002 };

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    rpc.setSender(client.RequestSender.init(
        &context,
        customRequestSender,
    ));
    ...
    const slot = try rpc.getSlot(.processed);
    try std.testing.expectEqual(@as(u64, 2003), slot);
}

test "root.setSenderAndRequestSenderOptions aliases setRequestSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 2004 };

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    rpc.setSenderAndRequestSenderOptions(
        client.RequestSender.init(&context, customRequestSender),
        .{
            .endpoint = "custom://set-sender-short-alias",
            .commitment = .confirmed,
            .request_timeout_ms = 73,
            .confirm_transaction_initial_timeout_ms = 83,
        },
    );

    try std.testing.expect(rpc.hasRequestSender());
    try std.testing.expectEqualStrings("custom://set-sender-short-alias", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 73), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 83), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 2005), slot);
    try std.testing.expect(context.saw_confirmed_commitment);
}

test "root.replaceOwnedMockAndRequestSenderOptions aliases replaceWithOwnedMockSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 1612 };
    ...
    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(1812);

    try rpc.replaceOwnedMockAndRequestSenderOptions(
        sender,
        .{
            .endpoint = "custom://replace-owned-mock-short-alias",
            .commitment = .finalized,
            .request_timeout_ms = 114,
            .confirm_transaction_initial_timeout_ms = 124,
        },
    );

    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://replace-owned-mock-short-alias", rpc.url());
    try std.testing.expectEqual(client.Commitment.finalized, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 114), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 124), rpc.getConfirmTransactionInitialTimeoutMs());
...
test "root.setBorrowedMock aliases setBorrowedMockSender" {
    const allocator = std.testing.allocator;
    var sender = client.MockSender.init(allocator);
    defer sender.deinit();
    try sender.pushSlotResult(2103);

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    rpc.setBorrowedMock(&sender);

    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expect(rpc.isMock());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 2103), slot);
}
...
test "root.setMockResponses aliases setMock" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setMockResponses(&.{});
    try rpc.pushMockSlotResult(2153);

    try std.testing.expect(rpc.isDirectMockClient());
    try std.testing.expect(rpc.isMock());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 2153), slot);
}
...
test "root.setMockResponsesAndRequestSenderOptions aliases setMockAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setMockResponsesAndRequestSenderOptions(
        &.{ .{ .result_json = "2154" } },
        .{
            .endpoint = "custom://set-mock-responses",
            .commitment = .confirmed,
            .request_timeout_ms = 54,
            .confirm_transaction_initial_timeout_ms = 74,
        },
    ...
    try std.testing.expect(rpc.isDirectMockClient());
    try std.testing.expectEqualStrings("custom://set-mock-responses", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 54), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 74), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(.confirmed);
    try std.testing.expectEqual(@as(u64, 2154), slot);
}
...
test "root.setMockRequestResponses aliases setMockRequestSender" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setMockRequestResponses(&.{});
    try rpc.pushMockSlotResult(2163);
    ...
    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 2163), slot);
}
...
test "root.setMockRequestMockSender aliases setMockRequestSenderWithSender" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(2164);

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setMockRequestMockSender(sender);
    ...
    const slot = try rpc.getSlot(.confirmed);
    try std.testing.expectEqual(@as(u64, 2164), slot);
}

test "root.setMockRequestMockSenderAndRequestSenderOptions aliases setMockRequestSenderWithSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(2165);

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setMockRequestMockSenderAndRequestSenderOptions(
        sender,
        .{
            .endpoint = "custom://set-mock-request-mock-sender",
            .commitment = .processed,
            .request_timeout_ms = 47,
            .confirm_transaction_initial_timeout_ms = 67,
        },
    ...
    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://set-mock-request-mock-sender", rpc.url());
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 47), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 67), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 2165), slot);
}

test "root.setMockRequestResponsesAndRequestSenderOptions aliases setMockRequestSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setMockRequestResponsesAndRequestSenderOptions(
        &.{ .{ .result_json = "2164" } },
        .{
            .endpoint = "custom://set-mock-request-responses",
            .commitment = .processed,
            .request_timeout_ms = 57,
            .confirm_transaction_initial_timeout_ms = 77,
        },
    );

    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://set-mock-request-responses", rpc.url());
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 57), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 77), rpc.getConfirmTransactionInitialTimeoutMs());
...
test "root.setMockRequestHandler aliases setMockRequestSenderWithHandler" {
    const allocator = std.testing.allocator;
    var handler_context = MockHandlerContext{};
...
test "root.setMockRequestHandlerAndRequestSenderOptions aliases setMockRequestSenderWithHandlerAndRequestSenderOptions" {
    const allocator = std.testing.allocator;
    var handler_context = MockHandlerContext{};
    ...
    try rpc.setMockRequestHandlerAndRequestSenderOptions(
        .{
            .context = &handler_context,
            .callback = dynamicMockHandler,
        },
        .{
            .endpoint = "custom://set-mock-request-handler-short",
            .commitment = .processed,
            .request_timeout_ms = 56,
            .confirm_transaction_initial_timeout_ms = 76,
        },
    );

    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://set-mock-request-handler-short", rpc.url());
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 56), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 76), rpc.getConfirmTransactionInitialTimeoutMs());
...
test "root.setOwnedMockAndRequestSenderOptions aliases setOwnedMockSenderAndRequestSenderOptions" {
    const allocator = std.testing.allocator;

    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(2203);

    var rpc = try client.RpcClient.new(allocator, "http://127.0.0.1:8899");
    defer rpc.deinit();

    try rpc.setOwnedMockAndRequestSenderOptions(
        sender,
        .{
            .endpoint = "custom://set-owned-mock-short-alias",
            .commitment = .confirmed,
            .request_timeout_ms = 115,
            .confirm_transaction_initial_timeout_ms = 125,
        },
    );

    try std.testing.expect(rpc.isRequestSenderBackedMockClient());
    try std.testing.expectEqualStrings("custom://set-owned-mock-short-alias", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 115), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 125), rpc.getConfirmTransactionInitialTimeoutMs());

    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 2203), slot);
}
...
test "root.RequestSender boolean mock helpers reflect mock state" {
    const allocator = std.testing.allocator;
    var mock_sender = client.MockSender.init(allocator);
    defer mock_sender.deinit();

    var request_sender = client.RequestSender.fromMockSender(&mock_sender);
    try std.testing.expect(!request_sender.hasMockResponses());
    try std.testing.expect(!request_sender.hasCapturedMockRequests());
    try std.testing.expect(!request_sender.hasMockRoutes());
    try std.testing.expect(!request_sender.hasPendingMockScriptedDispatches());
...
test "root.RequestSender route helpers expose scripted route surface" {
    ...
    try mock_sender_assertions.expectMockRequestSenderMatchedRouteCount(&request_sender, 0);
    try mock_sender_assertions.expectMockRequestSenderPendingScriptedDispatchCount(&request_sender, 3);
    try std.testing.expect(request_sender.hasMockRoutes());
    ...
    try mock_sender_assertions.expectMockRequestSenderPendingScriptedDispatchCount(&request_sender, 0);
    try std.testing.expect(!request_sender.hasMockRoutes());
    try std.testing.expect(request_sender.hasMatchedMockRoutes());
    try std.testing.expect(request_sender.hasMockRouteMatch("finalized-slot"));
...
test "root.RequestSender.replaceCallback aliases replaceWithCallback" {
    const allocator = std.testing.allocator;
    var mock_sender = client.MockSender.init(allocator);
    defer mock_sender.deinit();
    try mock_sender.pushSlotResult(1);
    var callback_context = RequestSenderContext{ .base_slot = 3352 };
    ...
    const sender = try rpc.requestSender();
    sender.replaceCallback(allocator, &callback_context, customRequestSender);

    try std.testing.expect(rpc.isCallbackRequestSenderClient());
    try std.testing.expect(!rpc.isMock());

    const slot = try rpc.getSlot(.processed);
    try std.testing.expectEqual(@as(u64, 3353), slot);
}
...
test "root.RequestSender.replaceCallbackAndDeinit aliases replaceWithCallbackAndDeinit" {
    const allocator = std.testing.allocator;
    var first_context = RequestSenderContext{ .base_slot = 3372 };
    var second_context = RequestSenderContext{ .base_slot = 3382 };
    ...
    const sender = try rpc.requestSender();
    sender.replaceCallbackAndDeinit(
        allocator,
        &second_context,
        customRequestSender,
        customRequestSenderDeinit,
    ...
    const slot = try rpc.getSlot(.processed);
    try std.testing.expectEqual(@as(u64, 3383), slot);
}
...
test "root.RequestSender.replaceMock aliases replaceWithMock" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 3384 };
    ...
    const sender = try rpc.requestSender();
    try sender.replaceMock(allocator, &.{});
    try sender.pushMockSlotResult(3385);
...
test "root.RequestSender.replaceMockHandler aliases replaceWithMockHandler" {
    const allocator = std.testing.allocator;
    var context = RequestSenderContext{ .base_slot = 3392 };
    var handler_context = MockHandlerContext{};
    ...
    const sender = try rpc.requestSender();
    try sender.replaceMockHandler(allocator, .{
        .context = &handler_context,
        .callback = dynamicMockHandler,
    });
...
test "root.RequestSender.initCallbackDeinit aliases initCallbackWithDeinit" {
    ...
    var rpc = try client.RpcClient.newWithRequestSender(
        allocator,
        client.RequestSender.initCallbackDeinit(
            &context,
            customRequestSender,
            customRequestSenderDeinit,
        ),
    ...
    const slot = try rpc.getSlot(.processed);
    try std.testing.expectEqual(@as(u64, 3395), slot);

    rpc.deinit();
    try std.testing.expectEqual(@as(usize, 1), context.deinit_count);
}

test "root.RequestSender.initMockHandler aliases initMockWithHandler" {
    const allocator = std.testing.allocator;
    var handler_context = MockHandlerContext{};

    var rpc = try client.RpcClient.newWithRequestSender(
        allocator,
        try client.RequestSender.initMockHandler(allocator, .{
            .context = &handler_context,
            .callback = dynamicMockHandler,
        }),
    );
...
test "root.RequestSender.initBorrowedMock aliases initBorrowedMockSender" {
    ...
    var mock_sender = client.MockSender.init(allocator);
    defer mock_sender.deinit();
    try mock_sender.pushSlotResult(3396);

    var rpc = try client.RpcClient.newWithRequestSender(
        allocator,
        client.RequestSender.initBorrowedMock(&mock_sender),
    );
    defer rpc.deinit();
    ...
    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 3396), slot);
}

test "root.RequestSender.initOwnedMock aliases initOwnedMockSender" {
    const allocator = std.testing.allocator;
    var owned_mock = client.MockSender.init(allocator);
    try owned_mock.pushSlotResult(3397);

    var rpc = try client.RpcClient.newWithRequestSender(
        allocator,
        try client.RequestSender.initOwnedMock(allocator, owned_mock),
    );
    defer rpc.deinit();
...
test "root.RequestSender mock script helpers expose pending counts and summary" {
    ...
    try std.testing.expectEqual(@as(usize, 2), request_sender.mockResponseCount());
    try mock_sender_assertions.expectMockRequestSenderPendingScriptedDispatchCount(&request_sender, 2);
...
test "root.RequestSender.fromMockSender borrows scripted mock sender state" {
...

    {
        ...
        try mock_sender_assertions.expectMockSenderFirstCapturedRequestId(&sender, 1);
        try mock_sender_assertions.expectMockRpcFirstCapturedRequestId(&rpc, 1);
        try mock_sender_assertions.expectMockSenderLastCapturedRequestId(&sender, 1);
        try mock_sender_assertions.expectMockRpcLastCapturedRequestId(&rpc, 1);
        try std.testing.expectEqualStrings("getSlot", sender.capturedRequests()[0].method);
        try std.testing.expectEqualStrings("getSlot", sender.firstCapturedRequestMethod().?);
        try std.testing.expectEqualStrings("getSlot", rpc.firstCapturedMockRequestMethod().?);
        ...
        try mock_sender_assertions.expectMockSenderFirstCapturedRequestMethod(&sender, "getSlot");
        try mock_sender_assertions.expectMockRpcFirstCapturedRequestMethod(&rpc, "getSlot");
        ...
        try std.testing.expect(std.mem.indexOf(u8, sender.firstCapturedRequestParamsJson().?, "\"processed\"") != null);
        try std.testing.expect(std.mem.indexOf(u8, rpc.firstCapturedMockRequestParamsJson().?, "\"processed\"") != null);
        try mock_sender_assertions.expectMockSenderFirstCapturedRequestParamsJsonContains(&sender, "\"processed\"");
        try mock_sender_assertions.expectMockRpcFirstCapturedRequestParamsJsonContains(&rpc, "\"processed\"");
        ...
        try std.testing.expect(std.mem.indexOf(u8, sender.firstCapturedRequestBody().?, "\"method\":\"getSlot\"") != null);
        try std.testing.expect(std.mem.indexOf(u8, rpc.firstCapturedMockRequestBody().?, "\"method\":\"getSlot\"") != null);
        try mock_sender_assertions.expectMockSenderFirstCapturedRequestBodyContains(&sender, "\"method\":\"getSlot\"");
        try mock_sender_assertions.expectMockRpcFirstCapturedRequestBodyContains(&rpc, "\"method\":\"getSlot\"");
        ...
        try std.testing.expect(std.mem.indexOf(u8, rpc.lastCapturedMockRequestBody().?, "\"method\":\"getSlot\"") != null);
    ...
    try std.testing.expectEqualStrings("getSlot", sender.firstCapturedRequestMethod().?);
    try std.testing.expectEqualStrings("getSlot", second_rpc.firstCapturedMockRequestMethod().?);
    try mock_sender_assertions.expectMockSenderFirstCapturedRequestId(&sender, 1);
    try mock_sender_assertions.expectMockRpcFirstCapturedRequestId(&second_rpc, 1);
    try mock_sender_assertions.expectMockSenderFirstCapturedRequestMethod(&sender, "getSlot");
    try mock_sender_assertions.expectMockRpcFirstCapturedRequestMethod(&second_rpc, "getSlot");
    try mock_sender_assertions.expectMockSenderLastCapturedRequestId(&sender, 1);
    try mock_sender_assertions.expectMockRpcLastCapturedRequestId(&second_rpc, 1);
    try std.testing.expectEqualStrings("getSlot", sender.lastCapturedRequestMethod().?);
    ...
    try mock_sender_assertions.expectMockRequestSenderFirstCapturedRequestId(last_request_sender, 1);
    try mock_sender_assertions.expectMockRequestSenderFirstCapturedRequestMethod(last_request_sender, "getSlot");
    ...
    try std.testing.expect(std.mem.indexOf(u8, sender.firstCapturedRequestParamsJson().?, "\"processed\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, second_rpc.firstCapturedMockRequestParamsJson().?, "\"processed\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, last_request_sender.firstCapturedMockRequestParamsJson().?, "\"processed\"") != null);
    try mock_sender_assertions.expectMockSenderFirstCapturedRequestParamsJsonContains(&sender, "\"processed\"");
    try mock_sender_assertions.expectMockRpcFirstCapturedRequestParamsJsonContains(&second_rpc, "\"processed\"");
    try mock_sender_assertions.expectMockRequestSenderFirstCapturedRequestParamsJsonContains(last_request_sender, "\"processed\"");
    try mock_sender_assertions.expectMockSenderLastCapturedRequestParamsJsonContains(&sender, "\"processed\"");
    ...
    try std.testing.expect(std.mem.indexOf(u8, sender.firstCapturedRequestBody().?, "\"method\":\"getSlot\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, second_rpc.firstCapturedMockRequestBody().?, "\"method\":\"getSlot\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, last_request_sender.firstCapturedMockRequestBody().?, "\"method\":\"getSlot\"") != null);
    try mock_sender_assertions.expectMockSenderFirstCapturedRequestBodyContains(&sender, "\"method\":\"getSlot\"");
    try mock_sender_assertions.expectMockRpcFirstCapturedRequestBodyContains(&second_rpc, "\"method\":\"getSlot\"");
    try mock_sender_assertions.expectMockRequestSenderFirstCapturedRequestBodyContains(last_request_sender, "\"method\":\"getSlot\"");
    try mock_sender_assertions.expectMockSenderLastCapturedRequestBodyContains(&sender, "\"method\":\"getSlot\"");
    ...
    try std.testing.expect(std.mem.indexOf(u8, sender.lastCapturedRequestBody().?, "\"method\":\"getSlot\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, second_rpc.lastCapturedMockRequestBody().?, "\"method\":\"getSlot\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, last_request_sender.lastCapturedMockRequestBody().?, "\"method\":\"getSlot\"") != null);
}
...
test "root.newBorrowedMock aliases borrowed mock sender constructor" {
    const allocator = std.testing.allocator;
    var sender = client.MockSender.init(allocator);
    defer sender.deinit();
    try sender.pushSlotResult(705);

    var rpc = try client.RpcClient.newBorrowedMock(allocator, &sender);
    defer rpc.deinit();
    ...
    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 705), slot);
}
...
test "root.newBorrowedMockAndRequestSenderOptions aliases borrowed request-sender constructor" {
    const allocator = std.testing.allocator;
    var sender = client.MockSender.init(allocator);
    defer sender.deinit();
    try sender.pushSlotResult(706);

    var rpc = try client.RpcClient.newBorrowedMockAndRequestSenderOptions(
        allocator,
        &sender,
        .{
            .endpoint = "custom://borrowed-mock-short-alias",
            .commitment = .processed,
            .request_timeout_ms = 8_300,
            .confirm_transaction_initial_timeout_ms = 9_300,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://borrowed-mock-short-alias", rpc.url());
    try std.testing.expectEqual(client.Commitment.processed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 8_300), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 9_300), rpc.getConfirmTransactionInitialTimeoutMs());
...
test "root.newOwnedMock aliases owned mock sender constructor" {
    const allocator = std.testing.allocator;
    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(8893);

    var rpc = try client.RpcClient.newOwnedMock(allocator, sender);
    defer rpc.deinit();
    ...
    const slot = try rpc.getSlot(null);
    try std.testing.expectEqual(@as(u64, 8893), slot);
}
...
test "root.newOwnedMockAndRequestSenderOptions aliases owned request-sender constructor" {
    const allocator = std.testing.allocator;
    var sender = client.MockSender.init(allocator);
    try sender.pushSlotResult(8894);

    var rpc = try client.RpcClient.newOwnedMockAndRequestSenderOptions(
        allocator,
        sender,
        .{
            .endpoint = "custom://owned-mock-short-alias",
            .commitment = .confirmed,
            .request_timeout_ms = 8_400,
            .confirm_transaction_initial_timeout_ms = 9_400,
        },
    );
    defer rpc.deinit();

    try std.testing.expectEqualStrings("custom://owned-mock-short-alias", rpc.url());
    try std.testing.expectEqual(client.Commitment.confirmed, rpc.getDefaultCommitment().?);
    try std.testing.expectEqual(@as(?u64, 8_400), rpc.getRequestTimeoutMs());
    try std.testing.expectEqual(@as(?u64, 9_400), rpc.getConfirmTransactionInitialTimeoutMs());
...
test "root.RpcClient boolean mock helpers reflect mock state" {
    ...
    try std.testing.expect(!rpc.hasMockResponses());
    ...
    try mock_sender_assertions.expectMockRpcMatchedRouteCount(&rpc, 0);
    try mock_sender_assertions.expectMockRpcPersistentRouteCount(&rpc, 0);
