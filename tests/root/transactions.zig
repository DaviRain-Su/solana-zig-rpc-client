...
test "root.buildOwnedVersionedNonceTransferMessageWithSenderAndOptions supports distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
    ...
    const nonce_blockhash = [_]u8{0x57} ** 32;
    const nonce_blockhash_base58 = try encodeBase58(allocator, &nonce_blockhash);
    defer allocator.free(nonce_blockhash_base58);
    ...
    var owned = try rpc.buildOwnedVersionedNonceTransferMessageWithSenderAndOptions(
        fee_payer_secret_key_base58,
        sender_secret_key_base58,
        nonce_authority_secret_key_base58,
        nonce_account_base58,
...
test "root.buildVersionedNonceTransferMessageBytesWithSenderAndOptions supports distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
    ...
    const nonce_blockhash = [_]u8{0x58} ** 32;
    const nonce_blockhash_base58 = try encodeBase58(allocator, &nonce_blockhash);
    defer allocator.free(nonce_blockhash_base58);
    ...
    const encoded_bytes = try rpc.buildVersionedNonceTransferMessageBytesWithSenderAndOptions(
        fee_payer_secret_key_base58,
        sender_secret_key_base58,
        nonce_authority_secret_key_base58,
        nonce_account_base58,
...
test "root.buildVersionedNonceTransferMessageBase64WithSender supports distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
...
test "root.getFeeForVersionedNonceTransferMessageWithSender supports fixed blockhash with distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
    ...
    const recent_blockhash = [_]u8{0x5b} ** 32;
    const recent_blockhash_base58 = try encodeBase58(allocator, &recent_blockhash);
    defer allocator.free(recent_blockhash_base58);
    ...
    var rpc = try RpcClient.newMock(allocator, &.{
        .{ .json = "{\"jsonrpc\":\"2.0\",\"result\":{\"context\":{\"slot\":321},\"value\":7100},\"id\":1}" },
    });
    defer rpc.deinit();

    const fee = try rpc.getFeeForVersionedNonceTransferMessageWithSender(
        fee_payer_secret_key_base58,
        sender_secret_key_base58,
        nonce_authority_secret_key_base58,
        nonce_account_base58,
    ...
    try std.testing.expectEqual(@as(?u64, 7100), fee.value);
    try std.testing.expectEqual(@as(usize, 1), rpc.mockRequestCount());
...
test "root.buildVersionedTransactionV0 reuses buildSignedVersionedTransactionV0" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedTransactionV0(
        allocator,
        payer.public_key,
        Hash.fromBytes(.{0x60} ** 32),
        instructions[0..],
...
test "root.buildVersionedMessageV0 reuses compileVersionedMessageV0" {
    const allocator = std.testing.allocator;
    ...
    var built = try client.buildVersionedMessageV0(
        allocator,
        payer,
        Hash.fromBytes(.{0x80} ** 32),
        instructions[0..],
        &.{},
    );
...
test "root.buildVersionedTransactionV0WithNonceInstructions reuses buildSignedVersionedTransactionV0WithNonceInstructions" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedTransactionV0WithNonceInstructions(
        allocator,
        payer.public_key,
        nonce_account,
        payer.public_key,
...
test "root.buildVersionedMessageV0WithNonceInstructions reuses buildOwnedVersionedMessageV0WithNonceInstructions" {
    const allocator = std.testing.allocator;
    ...
    var built = try client.buildVersionedMessageV0WithNonceInstructions(
        allocator,
        payer,
        nonce_account,
        payer,
        Hash.fromBytes(.{0x81} ** 32),
        instructions[0..],
        &.{},
    );
    defer built.deinit(allocator);
...
test "root.buildLegacyTransaction reuses buildSignedLegacyTransaction" {
    const allocator = std.testing.allocator;
    ...
    const payer = try Keypair.fromSecretKeyBytes(payer_secret_key);
    const destination = Pubkey.fromBytes(destination_raw.public_key.toBytes());
    const recent_blockhash = Hash.fromBytes(.{0x70} ** 32);
    const transfer = SystemProgram.transfer(payer.public_key, destination, 1_000);
    const instructions = [_]Instruction{transfer.instruction()};

    var signed = try client.buildLegacyTransaction(
        allocator,
        payer.public_key,
        recent_blockhash,
        instructions[0..],
...
test "root.buildLegacyTransactionWithNonceInstructions reuses buildSignedLegacyTransactionWithNonceInstructions" {
    const allocator = std.testing.allocator;
    ...
    const nonce_account = Pubkey.fromBytes(nonce_raw.public_key.toBytes());
    const destination = Pubkey.fromBytes(destination_raw.public_key.toBytes());
    const recent_blockhash = Hash.fromBytes(.{0x71} ** 32);
    const transfer = SystemProgram.transfer(payer.public_key, destination, 1_000);
    const instructions = [_]Instruction{transfer.instruction()};

    var signed = try client.buildLegacyTransactionWithNonceInstructions(
        allocator,
        payer.public_key,
        nonce_account,
        nonce_authority.public_key,
    ...
    const expected = try client.buildLegacyTransactionBase64WithNonceInstructions(
        allocator,
        payer.public_key,
        nonce_account,
        nonce_authority.public_key,
...
test "root.buildVersionedTransaction reuses buildSignedVersionedTransaction" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedTransaction(
        allocator,
        payer.public_key,
        Hash.fromBytes(.{0x72} ** 32),
        instructions[0..],
        &.{},
        &.{payer},
    );
    ...
    const expected = try client.buildVersionedTransactionBase64(
        allocator,
        payer.public_key,
        Hash.fromBytes(.{0x72} ** 32),
        instructions[0..],
...
test "root.buildVersionedTransactionWithNonceInstructions reuses buildSignedVersionedTransactionWithNonceInstructions" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedTransactionWithNonceInstructions(
        allocator,
        payer.public_key,
        nonce_account,
        payer.public_key,
        Hash.fromBytes(.{0x73} ** 32),
        instructions[0..],
        &.{},
        &.{payer},
    );
    ...
    const expected = try client.buildVersionedTransactionBase64WithNonceInstructions(
        allocator,
        payer.public_key,
        nonce_account,
        payer.public_key,
...
test "root.buildOwnedVersionedMessage reuses v0 helper" {
    const allocator = std.testing.allocator;
    ...
    var owned = try client.buildOwnedVersionedMessage(
        allocator,
        payer,
        Hash.fromBytes(.{0x64} ** 32),
        instructions[0..],
        &.{},
    );
    defer owned.deinit(allocator);

    const encoded = try owned.toBase64(allocator);
    defer allocator.free(encoded);
...
test "root.buildVersionedMessageBase64 reuses v0 helper" {
    const allocator = std.testing.allocator;
    ...
    const encoded = try client.buildVersionedMessageBase64(
        allocator,
        payer,
        Hash.fromBytes(.{0x65} ** 32),
        instructions[0..],
        &.{},
    );
    defer allocator.free(encoded);
...
test "root.buildOwnedVersionedMessageWithNonceInstructions reuses v0 helper" {
    const allocator = std.testing.allocator;
    ...
    var owned = try client.buildOwnedVersionedMessageWithNonceInstructions(
        allocator,
        payer,
        nonce_account,
        payer,
        Hash.fromBytes(.{0x66} ** 32),
        instructions[0..],
        &.{},
    );
    defer owned.deinit(allocator);
...
test "root.buildVersionedMessageBase64WithNonceInstructions reuses v0 helper" {
    const allocator = std.testing.allocator;
    ...
    const encoded = try client.buildVersionedMessageBase64WithNonceInstructions(
        allocator,
        payer,
        nonce_account,
        payer,
        Hash.fromBytes(.{0x67} ** 32),
        instructions[0..],
        &.{},
    );
    defer allocator.free(encoded);
...
test "root.buildOwnedLegacyNonceTransferMessageWithSender reuses buildLegacyNonceTransferMessageBase64WithSender" {
    const allocator = std.testing.allocator;
    ...
    var built = try client.buildOwnedLegacyNonceTransferMessageWithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
    ...
    const expected = try client.buildLegacyNonceTransferMessageBase64WithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
...
test "root.buildLegacyNonceTransferMessageBytesWithSender reuses buildLegacyNonceTransferMessageBase64WithSender" {
    const allocator = std.testing.allocator;
    ...
    const encoded_bytes = try client.buildLegacyNonceTransferMessageBytesWithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
...
test "root.buildNonceTransferMessageBase64WithSenderAndOptions supports distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
    ...
    const nonce_blockhash = [_]u8{0x5d} ** 32;
    const nonce_blockhash_base58 = try encodeBase58(allocator, &nonce_blockhash);
    defer allocator.free(nonce_blockhash_base58);
    ...
    const encoded = try rpc.buildNonceTransferMessageBase64WithSenderAndOptions(
        fee_payer_secret_key_base58,
        sender_secret_key_base58,
        nonce_authority_secret_key_base58,
        nonce_account_base58,
...
test "root.buildOwnedVersionedNonceTransferMessageWithSender reuses buildVersionedNonceTransferMessageBase64WithSender" {
    ...
    var built = try client.buildOwnedVersionedNonceTransferMessageWithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
    ...
    const expected = try client.buildVersionedNonceTransferMessageBase64WithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
...
test "root.buildVersionedNonceTransferMessageWithSender reuses buildOwnedVersionedNonceTransferMessageWithSender" {
    const allocator = std.testing.allocator;
    ...
    var built = try client.buildVersionedNonceTransferMessageWithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
...
test "root.buildVersionedNonceTransferMessageBytesWithSender reuses buildVersionedNonceTransferMessageBase64WithSender" {
    const allocator = std.testing.allocator;
    ...
    const message_bytes = try client.buildVersionedNonceTransferMessageBytesWithSender(
        allocator,
        Pubkey.fromBytes(fee_payer_raw.public_key.toBytes()),
        Pubkey.fromBytes(sender_raw.public_key.toBytes()),
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
...
test "root.buildVersionedTransferTransaction reuses buildSignedVersionedTransferTransaction" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedTransferTransaction(
        allocator,
        sender.public_key,
        Pubkey.fromBytes(destination_raw.public_key.toBytes()),
        Hash.fromBytes(.{0x74} ** 32),
        1_000,
        &.{},
        &.{sender},
    );
    ...
    const expected = try client.buildVersionedTransferTransactionBase64(
        allocator,
        sender.public_key,
        Pubkey.fromBytes(destination_raw.public_key.toBytes()),
        Hash.fromBytes(.{0x74} ** 32),
...
test "root.buildVersionedTransferTransactionWithNonce reuses buildSignedVersionedTransferTransactionWithNonce" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedTransferTransactionWithNonce(
        allocator,
        payer.public_key,
        Pubkey.fromBytes(nonce_raw.public_key.toBytes()),
        payer.public_key,
        Pubkey.fromBytes(destination_raw.public_key.toBytes()),
        Hash.fromBytes(.{0x75} ** 32),
        1_000,
        &.{},
        &.{payer},
    );
    ...
    const expected = try client.buildVersionedTransferTransactionBase64WithNonce(
        allocator,
        payer.public_key,
        Pubkey.fromBytes(nonce_raw.public_key.toBytes()),
        payer.public_key,
...
test "root.buildVersionedNonceTransferTransaction reuses buildSignedVersionedNonceTransferTransaction" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedNonceTransferTransaction(
        allocator,
        payer.public_key,
        Pubkey.fromBytes(nonce_raw.public_key.toBytes()),
        payer.public_key,
        Pubkey.fromBytes(destination_raw.public_key.toBytes()),
        Hash.fromBytes(.{0x76} ** 32),
        1_000,
        &.{},
        &.{payer},
    );
...
test "root.buildVersionedNonceTransferTransactionWithSender reuses buildSignedVersionedNonceTransferTransactionWithSender" {
    const allocator = std.testing.allocator;
    ...
    var signed = try client.buildVersionedNonceTransferTransactionWithSender(
        allocator,
        fee_payer.public_key,
        sender.public_key,
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
    ...
    const expected = try client.buildVersionedNonceTransferTransactionBase64WithSender(
        allocator,
        fee_payer.public_key,
        sender.public_key,
        Pubkey.fromBytes(nonce_account_raw.public_key.toBytes()),
...
test "root.getFeeForVersionedNonceTransferMessageWithSenderAndOptions supports fixed blockhash with distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
    ...
    var rpc = try RpcClient.newMock(allocator, &.{
        .{ .json = "{\"jsonrpc\":\"2.0\",\"result\":{\"context\":{\"slot\":456},\"value\":7200},\"id\":1}" },
    });
    defer rpc.deinit();

    const fee = try rpc.getFeeForVersionedNonceTransferMessageWithSenderAndOptions(
        fee_payer_secret_key_base58,
        sender_secret_key_base58,
        nonce_authority_secret_key_base58,
        nonce_account_base58,
    ...
    try std.testing.expectEqual(@as(?u64, 7200), fee.value);
    try std.testing.expectEqual(@as(usize, 1), rpc.mockRequestCount());
    try std.testing.expectEqualStrings("getFeeForMessage", rpc.capturedMockRequests()[0].method);
    try std.testing.expect(std.mem.indexOf(u8, rpc.capturedMockRequests()[0].request_body, expected_message) != null);
    try std.testing.expect(std.mem.indexOf(u8, rpc.capturedMockRequests()[0].params_json, "\"commitment\":\"processed\"") != null);
...
test "root.getFeeForNonceTransferMessageWithSenderAndOptions supports distinct payer sender and nonce authority" {
    const allocator = std.testing.allocator;
    ...
    const nonce_blockhash = [_]u8{0x5e} ** 32;
    const nonce_blockhash_base58 = try encodeBase58(allocator, &nonce_blockhash);
    defer allocator.free(nonce_blockhash_base58);
    ...
    try rpc.pushMockJsonResponse(response_body);
    try rpc.pushMockJsonResponse(
        "{\"jsonrpc\":\"2.0\",\"result\":{\"context\":{\"slot\":654},\"value\":7300},\"id\":1}",
    );

    const fee = try rpc.getFeeForNonceTransferMessageWithSenderAndOptions(
        fee_payer_secret_key_base58,
        sender_secret_key_base58,
        nonce_authority_secret_key_base58,
        nonce_account_base58,
    ...
    try std.testing.expectEqual(@as(?u64, 7300), fee.value);
    try std.testing.expectEqual(@as(usize, 2), rpc.mockRequestCount());
    try std.testing.expectEqualStrings("getAccountInfo", rpc.capturedMockRequests()[0].method);
    try std.testing.expect(std.mem.indexOf(u8, rpc.capturedMockRequests()[0].params_json, "\"encoding\":\"jsonParsed\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, rpc.capturedMockRequests()[0].params_json, "\"commitment\":\"confirmed\"") != null);
